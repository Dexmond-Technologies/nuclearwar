...
exports.clusterNews = clusterNews;
exports.clusterNewsHybrid = clusterNewsHybrid;
...
function clusterNews(items) {
    return (0, analysis_core_1.clusterNewsCore)(items, config_1.getSourceTier);
}
/**
 * Hybrid clustering: Jaccard first, then semantic refinement if ML available
 */
async function clusterNewsHybrid(items) {
    // Step 1: Fast Jaccard clustering
    const jaccardClusters = (0, analysis_core_1.clusterNewsCore)(items, config_1.getSourceTier);
    // Step 2: If ML unavailable or too few clusters, return Jaccard results
    if (!ml_worker_1.mlWorker.isAvailable || jaccardClusters.length < ml_config_1.ML_THRESHOLDS.minClustersForML) {
    ...
    try {
        ...
        const semanticGroups = await ml_worker_1.mlWorker.clusterBySemanticSimilarity(clusterTexts, ml_config_1.ML_THRESHOLDS.semanticClusterThreshold);
 ...
 */
function mergeSemanticallySimilarClusters(clusters, semanticGroups) {
    ...
    for (const group of semanticGroups) {
        ...
        const groupClusters = group
            ...
            .filter((c) => c !== undefined && !usedIds.has(c.id));
        ...
        const sortedByTier = [...groupClusters].sort((a, b) => {
            const tierA = (0, config_1.getSourceTier)(a.primarySource);
            const tierB = (0, config_1.getSourceTier)(b.primarySource);
            if (tierA !== tierB)
                return tierA - tierB;
