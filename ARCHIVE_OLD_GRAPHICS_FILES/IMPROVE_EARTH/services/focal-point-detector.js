...
exports.focalPointDetector = void 0;
...
class FocalPointDetector {
     ...
     */
    entityAppearsInTitle(entityId, title, index) {
        const entity = index.byId.get(entityId);
        if (!entity)
            return false;
        const titleLower = title.toLowerCase();
        // Check entity name
        if (titleLower.includes(entity.name.toLowerCase()))
            return true;
        // Check aliases
     ...
     */
    analyze(clusters, signalSummary) {
        const entityContexts = (0, entity_extraction_1.extractEntitiesFromClusters)(clusters);
     ...
     */
    aggregateEntities(entityContexts, clusters) {
     ...
     */
    buildFocalPoints(entityMentions, signalSummary) {
        const focalPoints = [];
        const index = (0, entity_index_1.getEntityIndex)();
     ...
     */
    createFocalPoint(mention, signals, _signalCountry) {
    ...
    calculateCorrelationBonus(mention, signals) {
        let bonus = 0;
    ...
    determineUrgency(score, signalTypeCount) {
        if (score > 70 || signalTypeCount >= 3)
            return 'critical';
        if (score > 50 || signalTypeCount >= 2)
            return 'elevated';
        return 'watch';
    }
    generateNarrative(mention, signals, signalTypes) {
        const parts = [];
    ...
    getCorrelationEvidence(mention, signals) {
        const evidence = [];
        if (mention.mentionCount > 0 && signals && signals.totalCount > 0) {
     ...
     */
    getSignalIcons(signalTypes) {
        return signalTypes.map(t => SIGNAL_TYPE_ICONS[t] || '').join(' ');
    }
    /**
     ...
     */
    getCountryUrgency(countryCode) {
        if (!this.lastSummary)
            return null;
        const fp = this.lastSummary.focalPoints.find(fp => fp.entityType === 'country' && fp.entityId === countryCode);
        return fp?.urgency || null;
    }
    /**
     * Get all country urgencies as a map (for batch CII calculation)
     */
    getCountryUrgencyMap() {
        const map = new Map();
        if (!this.lastSummary)
            return map;
     ...
     */
    getFocalPointForCountry(countryCode) {
        if (!this.lastSummary)
            return null;
        return this.lastSummary.focalPoints.find(fp => fp.entityType === 'country' && fp.entityId === countryCode) || null;
    }
    /**
     ...
     */
    getNewsCorrelationContext(countryCodes) {
        if (!this.lastSummary)
            return null;
        const relevantFPs = this.lastSummary.focalPoints.filter(fp => fp.entityType === 'country' && countryCodes.includes(fp.entityId) && fp.newsMentions > 0);
...
exports.focalPointDetector = new FocalPointDetector();
