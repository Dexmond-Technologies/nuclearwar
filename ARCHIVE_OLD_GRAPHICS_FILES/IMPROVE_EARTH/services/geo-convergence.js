...
exports.ingestGeoEvent = ingestGeoEvent;
exports.ingestProtests = ingestProtests;
exports.ingestFlights = ingestFlights;
exports.ingestVessels = ingestVessels;
exports.ingestEarthquakes = ingestEarthquakes;
exports.detectGeoConvergence = detectGeoConvergence;
exports.getLocationName = getLocationName;
exports.geoConvergenceToSignal = geoConvergenceToSignal;
exports.detectConvergence = detectConvergence;
exports.clearCells = clearCells;
exports.getCellCount = getCellCount;
exports.debugGetCells = debugGetCells;
exports.getAlertsNearLocation = getAlertsNearLocation;
...
function ingestGeoEvent(lat, lon, type, timestamp = new Date()) {
...
function ingestProtests(events) {
...
function getLocationName(lat, lon) {
    ...
    if (lat >= 25 && lat <= 40 && lon >= 25 && lon <= 75)
        return 'Middle East';
    if (lat >= 30 && lat <= 45 && lon >= 100 && lon <= 145)
        return 'East Asia';
    if (lat >= -10 && lat <= 25 && lon >= 90 && lon <= 130)
        return 'Southeast Asia';
    if (lat >= 35 && lat <= 70 && lon >= -10 && lon <= 40)
        return 'Europe';
    if (lat >= 44 && lat <= 75 && lon >= 20 && lon <= 180)
        return 'Russia';
    if (lat >= -35 && lat <= 35 && lon >= -20 && lon <= 55)
        return 'Africa';
    if (lat >= 25 && lat <= 50 && lon >= -125 && lon <= -65)
        return 'North America';
    if (lat >= -60 && lat <= 15 && lon >= -80 && lon <= -30)
        return 'South America';
    return `${lat.toFixed(1)}°, ${lon.toFixed(1)}°`;
}
function geoConvergenceToSignal(alert) {
...
function debugGetCells() {
    return new Map(cells);
}
function getAlertsNearLocation(lat, lon, radiusKm) {
    pruneOldEvents();
    let maxScore = 0;
    let maxTypes = 0;
