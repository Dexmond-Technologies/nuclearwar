...
exports.isAisConfigured = isAisConfigured;
exports.registerAisCallback = registerAisCallback;
exports.unregisterAisCallback = unregisterAisCallback;
exports.initAisStream = initAisStream;
exports.disconnectAisStream = disconnectAisStream;
exports.getAisStatus = getAisStatus;
exports.fetchAisSignals = fetchAisSignals;
const service_client_1 = require("@/generated/client/worldmonitor/maritime/v1/service_client");
const utils_1 = require("@/utils");
const data_freshness_1 = require("../data-freshness");
const runtime_config_1 = require("../runtime-config");
// ---- Proto fallback (desktop safety when relay URL is unavailable) ----
const client = new service_client_1.MaritimeServiceClient('', { fetch: (...args) => globalThis.fetch(...args) });
const snapshotBreaker = (0, utils_1.createCircuitBreaker)({ name: 'Maritime Snapshot', cacheTtlMs: 5 * 60 * 1000, persistCache: true });
...
function isAisConfigured() {
    return aisConfigured && (0, runtime_config_1.isFeatureAvailable)('aisRelay');
}
...
async function fetchRawRelaySnapshot(includeCandidates) {
...
async function fetchSnapshotPayload(includeCandidates) {
...
async function pollSnapshot(force = false) {
    ...
    if (!force && isClientRuntime && document.hidden)
        return;
    if (inFlight && !force)
        return;
    inFlight = true;
    try {
        ...
        if (itemCount > 0 || latestStatus.vessels > 0) {
            data_freshness_1.dataFreshness.recordUpdate('ais', itemCount > 0 ? itemCount : latestStatus.vessels);
        }
    }
    catch {
...
// ---- Exported Functions ----
function registerAisCallback(callback) {
    positionCallbacks.add(callback);
    startPolling();
}
function unregisterAisCallback(callback) {
...
async function fetchAisSignals() {
    if (!aisConfigured) {
