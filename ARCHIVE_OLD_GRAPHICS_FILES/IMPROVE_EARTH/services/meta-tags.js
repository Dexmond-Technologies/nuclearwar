...
exports.updateMetaTagsForStory = updateMetaTagsForStory;
exports.resetMetaTags = resetMetaTags;
exports.parseStoryParams = parseStoryParams;
exports.initMetaTags = initMetaTags;
const BASE_URL = 'https://worldmonitor.app';
const DEFAULT_IMAGE = 'https://worldmonitor.app/favico/og-image.png';
function updateMetaTagsForStory(meta) {
...
function resetMetaTags() {
    const defaultTitle = 'World Monitor by Dexmond Technologies – Global Situation with AI Insights';
...
function generateDescription(score, level, trend, type, countryName) {
    const parts = [];
    ...
    return `World Monitor ${parts.join('. ')}. Built by Dexmond Technologies Ltd.`;
}
...
function parseStoryParams(url) {
    ...
    if (!countryCode || !/^[A-Z]{2,3}$/i.test(countryCode))
        return null;
    const validTypes = ['ciianalysis', 'crisisalert', 'dailybrief', 'marketfocus'];
    const safeType = validTypes.includes(type)
        ? type
        : 'ciianalysis';
