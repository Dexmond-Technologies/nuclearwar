...
exports.analyzeFlightsForSurge = analyzeFlightsForSurge;
exports.getActiveSurges = getActiveSurges;
exports.getTheaterActivity = getTheaterActivity;
exports.detectForeignMilitaryPresence = detectForeignMilitaryPresence;
exports.foreignPresenceToSignal = foreignPresenceToSignal;
exports.getActiveForeignPresence = getActiveForeignPresence;
exports.surgeAlertToSignal = surgeAlertToSignal;
exports.getTheaterPostureSummaries = getTheaterPostureSummaries;
exports.recalcPostureWithVessels = recalcPostureWithVessels;
exports.getCriticalPostures = getCriticalPostures;
const bases_expanded_1 = require("@/config/bases-expanded");
...
const activeForeignPresence = new Map();
const seenForeignAlerts = new Set();
const THEATERS = [
    {
        id: 'middle-east',
...
function findNearbyBases(lat, lon) {
    const nearby = [];
    for (const base of bases_expanded_1.MILITARY_BASES_EXPANDED) {
...
function classifyFlight(flight) {
    if (isTransportFlight(flight))
        return 'transport';
    if (flight.aircraftType === 'fighter')
        return 'fighter';
    if (flight.aircraftType === 'reconnaissance' || flight.aircraftType === 'awacs')
        return 'recon';
    return 'other';
}
...
function analyzeFlightsForSurge(flights) {
    cleanupOldHistory();
    const theaterFlights = new Map();
...
function getActiveSurges() {
    return Array.from(activeSurges.values());
}
function getTheaterActivity(theaterId) {
...
function isHomeRegion(operator, regionId) {
    ...
    if (!config)
        return true; // Unknown operator - don't alert
...
function detectForeignMilitaryPresence(flights) {
    const newAlerts = [];
    ...
    for (const flight of flights) {
        ...
        if (isHomeRegion(flight.operator, region.id))
            continue;
...
const REGION_AFFECTED_COUNTRIES = {
    'persian-gulf': ['IR', 'SA'],
    'strait-hormuz': ['IR'],
    'iran-border': ['IR', 'IL'],
    'baltics': ['RU', 'UA'],
...
function foreignPresenceToSignal(alert) {
    const aircraftTypes = new Map();
    const callsigns = [];
    ...
    const isCritical = criticalCombos.some(([op, reg]) => alert.operator === op && alert.region.id === reg);
    ...
    if (operatorISO)
        relevantCountries.push(operatorISO);
    ...
    const newsContext = focal_point_detector_1.focalPointDetector.getNewsCorrelationContext(relevantCountries);
    ...
    for (const iso of relevantCountries) {
        const fp = focal_point_detector_1.focalPointDetector.getFocalPointForCountry(iso);
...
function getTheaterPostureSummaries(flights) {
    const summaries = [];
    for (const theater of POSTURE_THEATERS) {
        ...
        const postureLevel = total >= theater.thresholds.critical
            ...
            : total >= theater.thresholds.elevated
                ? 'elevated'
                : 'normal';
        const strikeCapable = byType.tankers >= theater.strikeIndicators.minTankers &&
        ...
        const parts = [];
        if (byType.fighters > 0)
            parts.push(`${byType.fighters} fighters`);
        if (byType.tankers > 0)
            parts.push(`${byType.tankers} tankers`);
        if (byType.awacs > 0)
            parts.push(`${byType.awacs} AWACS`);
        if (byType.reconnaissance > 0)
            parts.push(`${byType.reconnaissance} recon`);
 ...
 */
function recalcPostureWithVessels(postures) {
    for (const p of postures) {
        const theater = POSTURE_THEATERS.find((t) => t.id === p.theaterId);
        if (!theater)
            continue;
        const airLevel = p.totalAircraft >= theater.thresholds.critical ? 2
            : p.totalAircraft >= theater.thresholds.elevated ? 1 : 0;
        const navalLevel = p.totalVessels >= theater.navalThresholds.critical ? 2
        ...
        if (theater.targetNation) {
            ...
            if (code) {
                const cii = (0, country_instability_1.getCountryScore)(code);
                if (cii !== null) {
        ...
        if (p.totalAircraft > 0)
            parts.push(`${p.totalAircraft} aircraft`);
        if (p.totalVessels > 0)
            parts.push(`${p.totalVessels} vessels`);
...
function getCriticalPostures(flights) {
    return getTheaterPostureSummaries(flights).filter((p) => p.postureLevel === 'critical' || (p.postureLevel === 'elevated' && p.strikeCapable));
}
