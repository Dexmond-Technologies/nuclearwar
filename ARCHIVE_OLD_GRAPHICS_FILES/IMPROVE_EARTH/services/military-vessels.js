...
exports.initMilitaryVesselStream = initMilitaryVesselStream;
exports.disconnectMilitaryVesselStream = disconnectMilitaryVesselStream;
exports.getMilitaryVesselStatus = getMilitaryVesselStatus;
exports.fetchMilitaryVessels = fetchMilitaryVessels;
exports.getMilitaryVesselsStatus = getMilitaryVesselsStatus;
exports.getVesselByMmsi = getVesselByMmsi;
exports.getVesselsNearLocation = getVesselsNearLocation;
exports.getDarkVessels = getDarkVessels;
exports.isMilitaryVesselTrackingConfigured = isMilitaryVesselTrackingConfigured;
const utils_1 = require("@/utils");
const military_1 = require("@/config/military");
const maritime_1 = require("./maritime");
const usni_fleet_1 = require("./usni-fleet");
...
function analyzeMmsi(mmsi) {
    if (!mmsi || mmsi.length < 9)
        return { isPotentialMilitary: false };
    ...
    for (const pattern of military_1.MILITARY_VESSEL_PATTERNS) {
...
function matchKnownVessel(name) {
    ...
    for (const vessel of military_1.KNOWN_NAVAL_VESSELS) {
 ...
 */
function getAisShipTypeName(shipType) {
    ...
    // Direct match
    if (aisTypeMap[shipType])
        return aisTypeMap[shipType];
 ...
 */
function getVesselTypeFromAis(shipType) {
    // AIS ship type codes
    // 35 = Military ops
    // 50-59 = Special craft
    // 55 = Law enforcement
    if (shipType === 35)
        return 'destroyer'; // Generic military
    if (shipType === 55)
        return 'patrol'; // Law enforcement/coast guard
 ...
 */
function getNearbyBase(lat, lon) {
 ...
 */
function getNearbyChokepoint(lat, lon) {
 ...
 */
function processAisPosition(data) {
    ...
    const nearHotspot = (0, military_1.getNearbyHotspot)(lat, lon);
 ...
 */
function clusterVessels(vessels) {
    const clusters = [];
    ...
    for (const hotspot of military_1.MILITARY_HOTSPOTS) {
        ...
        if (nearbyVessels.length >= 2) {
            ...
            const hasCombatants = nearbyVessels.some((v) => v.vesselType === 'destroyer' || v.vesselType === 'frigate');
            let activityType = 'unknown';
            if (hasCarrier || nearbyVessels.length >= 5)
 ...
 */
function initMilitaryVesselStream() {
    if (isTracking)
        return;
    ...
    (0, maritime_1.registerAisCallback)(processAisPosition);
    isTracking = true;
    // Ensure AIS stream is running
    if ((0, maritime_1.isAisConfigured)()) {
        (0, maritime_1.initAisStream)();
    }
}
/**
 * Disconnect from vessel stream
 */
function disconnectMilitaryVesselStream() {
    if (!isTracking)
        return;
    (0, maritime_1.unregisterAisCallback)(processAisPosition);
    isTracking = false;
}
/**
 * Get current tracking status
 */
function getMilitaryVesselStatus() {
    return {
 ...
 */
async function fetchMilitaryVessels() {
    return breaker.execute(async () => {
        ...
        if (vesselCache && Date.now() - vesselCache.timestamp < CACHE_TTL) {
            ...
            if (!isTracking && (0, maritime_1.isAisConfigured)()) {
        ...
        try {
            const usniReport = await (0, usni_fleet_1.fetchUSNIFleetReport)();
            if (usniReport && usniReport.vessels.length > 0) {
                const merged = (0, usni_fleet_1.mergeUSNIWithAIS)(vessels, usniReport, aisClusters);
                return merged;
            }
        }
        catch (e) {
            console.warn('[Military Vessels] USNI merge failed, using AIS only:', e.message);
        }
 ...
 */
function getMilitaryVesselsStatus() {
 ...
 */
function getVesselsNearLocation(lat, lon, radiusDeg = 2) {
    const result = [];
 ...
 */
function getDarkVessels() {
    return Array.from(trackedVessels.values()).filter((v) => v.isDark);
}
/**
 * Check if AIS stream is configured
 */
function isMilitaryVesselTrackingConfigured() {
    return (0, maritime_1.isAisConfigured)();
}
