...
exports.parallelAnalysis = void 0;
...
class ParallelAnalysisService {
    ...
    async analyzeHeadlines(clusters) {
        this.analysisCount++;
        const analyzed = [];
        ...
        if (ml_worker_1.mlWorker.isAvailable) {
            const [s, e, emb] = await Promise.all([
                ml_worker_1.mlWorker.classifySentiment(titles).catch(() => null),
                ml_worker_1.mlWorker.extractEntities(titles).catch(() => null),
    ...
    scoreByKeywords(titleLower, _cluster) {
        let score = 0;
        const reasons = [];
    ...
    scoreByEntities(entities) {
        ...
        const geopoliticalLocations = locations.filter(e => FLASHPOINT_KEYWORDS.some(fp => e.text.toLowerCase().includes(fp)));
        let score = 0;
        const reasons = [];
    ...
    async scoreByNovelty(title, embedding) {
        let maxSimilarity = 0;
        let mostSimilar = '';
        for (const [recentTitle, recentEmb] of this.recentEmbeddings) {
            if (recentTitle === title)
                continue;
    ...
    scoreBySourceDiversity(cluster) {
    ...
    async getEmbeddings(titles) {
        return ml_worker_1.mlWorker.embedTexts(titles);
    }
    getLastReport() {
...
exports.parallelAnalysis = new ParallelAnalysisService();
