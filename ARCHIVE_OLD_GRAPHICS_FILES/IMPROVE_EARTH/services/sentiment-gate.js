...
exports.filterBySentiment = filterBySentiment;
 ...
 */
async function filterBySentiment(items, threshold = DEFAULT_THRESHOLD) {
    if (items.length === 0)
        return [];
    ...
    try {
        ...
        for (let i = 0; i < titles.length; i += BATCH_SIZE) {
            ...
            const batchResults = await ml_worker_1.mlWorker.classifySentiment(batch);
