...
exports.renderStoryToCanvas = renderStoryToCanvas;
...
async function renderStoryToCanvas(data) {
    ...
    const dateStr = new Date().toLocaleDateString((0, i18n_1.getLocale)(), { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' });
    ...
    if (data.cii?.components) {
        y += 44;
        const comps = [
            { label: (0, i18n_1.t)('common.unrest').toUpperCase(), val: data.cii.components.unrest, color: '#f97316' },
            { label: (0, i18n_1.t)('common.conflict').toUpperCase(), val: data.cii.components.conflict, color: '#dc2626' },
            { label: (0, i18n_1.t)('common.security').toUpperCase(), val: data.cii.components.security, color: '#ef4444' },
            { label: (0, i18n_1.t)('common.information').toUpperCase(), val: data.cii.components.information, color: '#8b5cf6' },
        ];
    ...
    if (data.news.length > 0 && y < FOOTER_Y - 200) {
        ...
        if (alertCount > 0)
            statsText += `  ·  ${alertCount} high-priority alerts`;
    ...
    if (data.theater && y < FOOTER_Y - 200) {
        ...
        if (data.theater.fighters || data.theater.tankers || data.theater.awacs) {
            ...
            ctx.font = '400 24px Inter, system-ui, sans-serif';
            const parts = [];
            if (data.theater.fighters)
                parts.push(`Fighters: ${data.theater.fighters}`);
            if (data.theater.tankers)
                parts.push(`Tankers: ${data.theater.tankers}`);
            if (data.theater.awacs)
                parts.push(`AWACS: ${data.theater.awacs}`);
...
function drawSeparator(ctx, y, pad) {
    ctx.strokeStyle = '#222';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(pad, y);
