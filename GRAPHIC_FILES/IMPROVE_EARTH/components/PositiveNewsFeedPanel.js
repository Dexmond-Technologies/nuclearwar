...
exports.PositiveNewsFeedPanel = void 0;
const Panel_1 = require("./Panel");
const positive_classifier_1 = require("@/services/positive-classifier");
const happy_share_renderer_1 = require("@/services/happy-share-renderer");
 ...
 */
class PositiveNewsFeedPanel extends Panel_1.Panel {
    constructor() {
        ...
        this.handleShareClick = (e) => {
            ...
            (0, happy_share_renderer_1.shareHappyCard)(item).catch(() => { });
    ...
    createFilterBar() {
        ...
        for (const category of positive_classifier_1.HAPPY_CATEGORY_ALL) {
            const btn = document.createElement('button');
            btn.className = 'positive-filter-btn';
            btn.textContent = positive_classifier_1.HAPPY_CATEGORY_LABELS[category];
    ...
    renderCard(item, idx) {
        const imageHtml = item.imageUrl
            ? `<div class="positive-card-image"><img src="${(0, sanitize_1.sanitizeUrl)(item.imageUrl)}" alt="" loading="lazy" onerror="this.parentElement.style.display='none'"></div>`
            : '';
        const categoryLabel = item.happyCategory ? positive_classifier_1.HAPPY_CATEGORY_LABELS[item.happyCategory] : '';
        const categoryBadgeHtml = item.happyCategory
            ? `<span class="positive-card-category cat-${(0, sanitize_1.escapeHtml)(item.happyCategory)}">${(0, sanitize_1.escapeHtml)(categoryLabel)}</span>`
            : '';
        return `<a class="positive-card" href="${(0, sanitize_1.sanitizeUrl)(item.link)}" target="_blank" rel="noopener" data-category="${(0, sanitize_1.escapeHtml)(item.happyCategory || '')}">
  ${imageHtml}
  <div class="positive-card-body">
    <div class="positive-card-meta">
      <span class="positive-card-source">${(0, sanitize_1.escapeHtml)(item.source)}</span>
      ${categoryBadgeHtml}
    </div>
    <span class="positive-card-title">${(0, sanitize_1.escapeHtml)(item.title)}</span>
    <span class="positive-card-time">${(0, utils_1.formatTime)(item.pubDate)}</span>
...
exports.PositiveNewsFeedPanel = PositiveNewsFeedPanel;
