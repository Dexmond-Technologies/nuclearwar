...
exports.createConvergenceAlert = createConvergenceAlert;
exports.createCIIAlert = createCIIAlert;
exports.createCascadeAlert = createCascadeAlert;
exports.checkCIIChanges = checkCIIChanges;
exports.calculateStrategicRiskOverview = calculateStrategicRiskOverview;
exports.getAlerts = getAlerts;
exports.getRecentAlerts = getRecentAlerts;
exports.clearAlerts = clearAlerts;
exports.getAlertCount = getAlertCount;
const geo_convergence_1 = require("./geo-convergence");
const country_instability_1 = require("./country-instability");
const country_geometry_1 = require("./country-geometry");
const i18n_1 = require("@/services/i18n");
const alerts = [];
const previousCIIScores = new Map();
...
function getPriorityFromCIIChange(change, level) {
    const absChange = Math.abs(change);
    // Match CII thresholds: critical at 81+, high at 66+
    if (level === 'critical')
        return 'critical';
    if (level === 'high' || absChange >= 30)
        return 'high';
    if (level === 'elevated' || absChange >= 15)
        return 'medium';
    return 'low';
}
function getPriorityFromCascadeImpact(impact, count) {
    if (impact === 'critical' || (impact === 'high' && count >= 3))
        return 'critical';
    if (impact === 'high' || count >= 5)
        return 'high';
    if (impact === 'medium' || count >= 3)
        return 'medium';
    return 'low';
}
function getPriorityFromConvergence(score, typeCount) {
    // Convergence: 4+ event types or score 90+ = critical, 3 types or 70+ = high
    if (typeCount >= 4 || score >= 90)
        return 'critical';
    if (typeCount >= 3 || score >= 70)
        return 'high';
    if (score >= 50)
        return 'medium';
    return 'low';
}
function buildConvergenceAlert(convergence, alertId) {
    ...
    return {
        ...
        title: (0, i18n_1.t)('alerts.geoAlert', { location }),
        summary: (0, i18n_1.t)('alerts.eventsDetected', { count: convergence.totalEvents, lat: convergence.lat.toFixed(1), lon: convergence.lon.toFixed(1) }),
...
function createConvergenceAlert(convergence) {
    const alertId = `conv-${convergence.cellId}`;
    const alert = buildConvergenceAlert(convergence, alertId);
    return addAndMergeAlert(alert);
}
function createCIIAlert(country, countryName, previousScore, currentScore, level, driver) {
    ...
    const alert = {
        ...
        title: (0, i18n_1.t)(change > 0 ? 'alerts.instabilityRising' : 'alerts.instabilityFalling', { country: countryName }),
        summary: (0, i18n_1.t)(summaryKey, { from: previousScore, to: currentScore, change: changeStr, driver }),
...
function createCascadeAlert(cascade) {
    if (cascade.countriesAffected.length === 0)
        return null;
    ...
    const alert = {
        ...
        title: (0, i18n_1.t)('alerts.infraAlert', { name: cascade.source.name }),
        summary: (0, i18n_1.t)('alerts.countriesAffected', { count: cascade.countriesAffected.length, impact: highestImpact }),
...
function shouldMergeAlerts(a, b) {
...
function getHigherPriority(a, b) {
...
function generateCompositeTitle(a, b) {
    const ciiChange = a.components.ciiChange || b.components.ciiChange;
    if (ciiChange) {
        return (0, i18n_1.t)(ciiChange.change > 0 ? 'alerts.instabilityRising' : 'alerts.instabilityFalling', { country: ciiChange.countryName });
    }
    if (a.components.convergence || b.components.convergence) {
        const countryCode = a.countries[0] || b.countries[0];
        const location = countryCode ? getCountryDisplayName(countryCode) : (0, i18n_1.t)('alerts.multipleRegions');
        return (0, i18n_1.t)('alerts.geoAlert', { location });
    }
    if (a.components.cascade || b.components.cascade) {
        return (0, i18n_1.t)('alerts.cascadeAlert');
    }
    const countryCode = a.countries[0] || b.countries[0];
    const location = countryCode ? getCountryDisplayName(countryCode) : (0, i18n_1.t)('alerts.multipleRegions');
    return (0, i18n_1.t)('alerts.alert', { location });
}
function generateCompositeSummary(a, b) {
    ...
    if (ciiA && ciiB && ciiA.country === ciiB.country) {
        ...
        return (0, i18n_1.t)(summaryKey, { from: earliest.previousScore, to: latest.currentScore, change: changeStr, driver: latest.driver });
    }
...
function addAndMergeAlert(alert) {
...
function getCountriesNearLocation(lat, lon) {
    const countries = [];
...
function checkCIIChanges() {
    ...
    for (const score of scores) {
        ...
        if (!inLearning && Math.abs(change) >= 10) {
            ...
            const alert = createCIIAlert(score.code, score.name, previous, score.score, score.level, driver);
            if (alert)
...
function getHighestComponent(score) {
    const { unrest, security, information } = score.components;
    if (unrest >= security && unrest >= information)
        return 'Civil Unrest';
...
function updateAlerts(convergenceAlerts) {
...
function calculateStrategicRiskOverview(convergenceAlerts) {
    const ciiScores = (0, country_instability_1.calculateCII)();
    ...
    const composite = Math.round(convergenceScore * convergenceWeight +
...
function calculateCIIRiskScore(scores) {
    if (scores.length === 0)
        return 0;
...
let previousCompositeScore = null;
function determineTrend(current) {
...
function countInfrastructureIncidents() {
    return alerts.filter(a => a.components.cascade).length;
}
function identifyTopRisks(convergence, cii) {
    const risks = [];
    const top = convergence[0];
    if (top) {
        const location = (0, geo_convergence_1.getLocationName)(top.lat, top.lon);
