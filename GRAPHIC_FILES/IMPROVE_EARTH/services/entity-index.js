...
exports.buildEntityIndex = buildEntityIndex;
exports.getEntityIndex = getEntityIndex;
exports.lookupEntityByAlias = lookupEntityByAlias;
exports.lookupEntitiesByKeyword = lookupEntitiesByKeyword;
exports.lookupEntitiesBySector = lookupEntitiesBySector;
exports.findRelatedEntities = findRelatedEntities;
exports.findEntitiesInText = findEntitiesInText;
exports.getEntityDisplayName = getEntityDisplayName;
const entities_1 = require("@/config/entities");
...
function getEntityIndex() {
    if (!cachedIndex) {
        cachedIndex = buildEntityIndex(entities_1.ENTITY_REGISTRY);
    }
    return cachedIndex;
}
function lookupEntityByAlias(alias) {
    const index = getEntityIndex();
    const id = index.byAlias.get(alias.toLowerCase());
    return id ? index.byId.get(id) : undefined;
}
function lookupEntitiesByKeyword(keyword) {
    const index = getEntityIndex();
...
function lookupEntitiesBySector(sector) {
    const index = getEntityIndex();
...
function findRelatedEntities(entityId) {
    const index = getEntityIndex();
