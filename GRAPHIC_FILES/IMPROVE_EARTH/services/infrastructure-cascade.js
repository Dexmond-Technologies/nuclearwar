...
exports.calculateCascade = calculateCascade;
exports.getCableById = getCableById;
...
function addCablesAsNodes(graph) {
    for (const cable of geo_1.UNDERSEA_CABLES) {
...
function addPipelinesAsNodes(graph) {
    for (const pipeline of pipelines_1.PIPELINES) {
...
function addPortsAsNodes(graph) {
    for (const port of ports_1.PORTS) {
...
function addChokepointsAsNodes(graph) {
...
function addCountriesAsNodes(graph) {
...
function addEdge(graph, edge) {
    ...
    if (!graph.outgoing.has(edge.from))
        graph.outgoing.set(edge.from, []);
    graph.outgoing.get(edge.from).push(edge);
    if (!graph.incoming.has(edge.to))
        graph.incoming.set(edge.to, []);
    graph.incoming.get(edge.to).push(edge);
}
function buildCableCountryEdges(graph) {
    for (const cable of geo_1.UNDERSEA_CABLES) {
...
function buildPipelineCountryEdges(graph) {
    for (const pipeline of pipelines_1.PIPELINES) {
...
function buildPortCountryEdges(graph) {
    for (const port of ports_1.PORTS) {
...
function getAffectedCountries(port) {
    ...
    if (port.id === 'port_said' || port.id === 'suez_port') {
        affected.push({ code: 'DE', strength: 0.6, reason: 'Major EU importer via Suez' }, { code: 'GB', strength: 0.5, reason: 'UK-Asia trade' }, { code: 'NL', strength: 0.5, reason: 'Rotterdam connection' }, { code: 'CN', strength: 0.4, reason: 'China-EU trade route' }, { code: 'IT', strength: 0.4, reason: 'Mediterranean trade' });
    }
    // Strait of Hormuz ports
    if (port.id === 'bandar_abbas' || port.id === 'fujairah' || port.id === 'ras_tanura') {
        affected.push({ code: 'JP', strength: 0.7, reason: 'Oil import dependency' }, { code: 'KR', strength: 0.6, reason: 'Oil import dependency' }, { code: 'IN', strength: 0.5, reason: 'Oil imports' }, { code: 'CN', strength: 0.5, reason: 'Oil imports' });
    }
    // Malacca Strait ports
    if (port.id === 'singapore' || port.id === 'klang' || port.id === 'tanjung_pelepas') {
        affected.push({ code: 'CN', strength: 0.6, reason: 'Trade route dependency' }, { code: 'JP', strength: 0.5, reason: 'Trade route' }, { code: 'KR', strength: 0.5, reason: 'Trade route' });
    }
    // Panama Canal ports
    if (port.id === 'colon' || port.id === 'balboa') {
        affected.push({ code: 'US', strength: 0.5, reason: 'East-West coast shipping' }, { code: 'CN', strength: 0.4, reason: 'Trade route to US East Coast' });
    }
    // Red Sea/Aden ports (especially relevant with Houthi disruptions)
    if (port.id === 'aden' || port.id === 'djibouti' || port.id === 'hodeidah') {
        affected.push({ code: 'DE', strength: 0.5, reason: 'Europe-Asia shipping route' }, { code: 'GB', strength: 0.5, reason: 'Shipping route' }, { code: 'IT', strength: 0.4, reason: 'Mediterranean access' }, { code: 'SA', strength: 0.4, reason: 'Regional trade' });
    }
    return affected;
}
function buildChokepointEdges(graph) {
    ...
    for (const waterway of geo_1.STRATEGIC_WATERWAYS) {
        ...
        const nearbyPorts = ports_1.PORTS.filter(port => {
...
function calculateCascade(sourceId, disruptionLevel = 1.0) {
    ...
    while (queue.length > 0) {
        ...
        for (const edge of dependents) {
            ...
            if (!targetNode || impactStrength < 0.05)
                continue;
...
function getCapacityForCountry(sourceId, countryCode, graph, dependencyChain) {
    if (sourceId.startsWith('cable:')) {
        ...
        const cable = geo_1.UNDERSEA_CABLES.find(c => c.id === cableId);
...
function findRedundancies(sourceId) {
    if (!sourceId.startsWith('cable:'))
        return [];
    const cableId = sourceId.replace('cable:', '');
    const sourceCable = geo_1.UNDERSEA_CABLES.find(c => c.id === cableId);
    if (!sourceCable)
        return [];
...
function getCableById(id) {
    return geo_1.UNDERSEA_CABLES.find(c => c.id === id);
}
function getPipelineById(id) {
    return pipelines_1.PIPELINES.find(p => p.id === id);
}
function getPortById(id) {
    return ports_1.PORTS.find((p) => p.id === id);
}
