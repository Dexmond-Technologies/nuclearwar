...
exports.PROGRESS_INDICATORS = void 0;
exports.fetchProgressData = fetchProgressData;
...
// ---- Data Fetching ----
async function fetchProgressDataFresh() {
    const results = await Promise.all(exports.PROGRESS_INDICATORS.map(async (indicator) => {
        try {
            const response = await (0, economic_1.getIndicatorData)(indicator.code, {
                countries: ['1W'],
...
async function fetchProgressData() {
    return breaker.execute(() => fetchProgressDataFresh(), exports.PROGRESS_INDICATORS.map(fallbackDataSet));
}
function emptyDataSet(indicator) {
    return {
        indicator,
        data: [],
        latestValue: 0,
...
function fallbackDataSet(indicator) {
