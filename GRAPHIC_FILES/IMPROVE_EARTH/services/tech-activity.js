...
exports.aggregateTechActivity = aggregateTechActivity;
exports.getTopActiveHubs = getTopActiveHubs;
exports.getHubActivity = getHubActivity;
const tech_hub_index_1 = require("./tech-hub-index");
const TIER_BONUS = {
    mega: 15,
    major: 8,
    emerging: 0,
};
function aggregateTechActivity(clusters) {
    const hubAccumulators = new Map();
    // Match each cluster to potential tech hubs
    for (const cluster of clusters) {
        const matches = (0, tech_hub_index_1.inferHubsFromTitle)(cluster.primaryTitle);
...
function getTopActiveHubs(clusters, limit = 10) {
    return aggregateTechActivity(clusters).slice(0, limit);
}
function getHubActivity(hubId, clusters) {
