...
exports.THREAT_LABELS = exports.THREAT_PRIORITY = exports.THREAT_COLORS = void 0;
exports.getThreatColor = getThreatColor;
exports.getThreatLabel = getThreatLabel;
exports.classifyByKeyword = classifyByKeyword;
exports.classifyWithAI = classifyWithAI;
exports.aggregateThreats = aggregateThreats;
...
function getThreatColor(level) {
    return (0, utils_1.getCSSColor)(THREAT_VAR_MAP[level] || '--text-dim');
}
exports.THREAT_PRIORITY = {
    critical: 5,
    high: 4,
    medium: 3,
    low: 2,
...
function getThreatLabel(level) {
    return (0, i18n_1.t)(`components.threatLabels.${level}`);
}
...
function shouldEscalateToCritical(lower, matchCat) {
    if (matchCat !== 'conflict' && matchCat !== 'military')
        return false;
    return ESCALATION_ACTIONS.test(lower) && ESCALATION_TARGETS.test(lower);
}
function classifyByKeyword(title, variant = 'full') {
...
const classifyClient = new service_client_1.IntelligenceServiceClient('', { fetch: (...args) => globalThis.fetch(...args) });
const VALID_LEVELS = {
    critical: 'critical', high: 'high', medium: 'medium', low: 'low', info: 'info',
};
function toThreat(resp) {
...
function flushBatch() {
    batchTimer = null;
    if (batchPaused || batchInFlight || batchQueue.length === 0)
        return;
    batchInFlight = true;
        ...
        try {
            for (let i = 0; i < batch.length; i++) {
                ...
                try {
                    ...
                    if (err instanceof service_client_1.ApiError && (err.statusCode === 429 || err.statusCode >= 500)) {
                        batchPaused = true;
                        const delay = err.statusCode === 429 ? 60000 : 30000;
...
function scheduleBatch() {
    if (batchTimer || batchPaused || batchInFlight || batchQueue.length === 0)
        return;
...
function classifyWithAI(title, variant) {
...
function aggregateThreats(items) {
    ...
    for (const item of withThreat) {
        const p = exports.THREAT_PRIORITY[item.threat.level];
        if (p > maxPriority) {
            maxPriority = p;
