...
exports.extractEntitiesWithML = extractEntitiesWithML;
exports.ingestHeadlines = ingestHeadlines;
exports.drainTrendingSignals = drainTrendingSignals;
exports.getTrendingConfig = getTrendingConfig;
exports.updateTrendingConfig = updateTrendingConfig;
exports.suppressTrendingTerm = suppressTrendingTerm;
exports.unsuppressTrendingTerm = unsuppressTrendingTerm;
exports.getTrackedTermCount = getTrackedTermCount;
...
const LEADER_PATTERNS = LEADER_NAMES.map(name => ({
    name,
    pattern: new RegExp(`\\b${(0, analysis_constants_1.escapeRegex)(name)}\\b`, 'i'),
}));
const termFrequency = new Map();
const seenHeadlines = new Map();
const pendingSignals = [];
const activeSpikeTerms = new Set();
const autoSummaryRuns = [];
...
function uniqueBlockedTerms(terms) {
...
function readConfig() {
    ...
    try {
        ...
        cachedConfig = sanitizeConfig(JSON.parse(raw));
    }
    catch {
        cachedConfig = { ...DEFAULT_CONFIG };
    }
...
function getBlockedTermSet(config) {
    return new Set([
        ...Array.from(analysis_constants_1.SUPPRESSED_TRENDING_TERMS).map(term => toTermKey(term)),
...
function normalizeMLEntityText(text) {
    return text
        .replace(/^##/, '')
        .replace(/\s+/g, ' ')
...
function dedupeEntityTerms(entities) {
...
async function extractMLEntitiesForTexts(texts) {
    if (!ml_worker_1.mlWorker.isAvailable || texts.length === 0) {
        return texts.map(() => []);
    }
    const entitiesByText = [];
    for (let i = 0; i < texts.length; i += ML_ENTITY_BATCH_SIZE) {
        const batch = texts.slice(i, i + ML_ENTITY_BATCH_SIZE);
        const batchResults = await ml_worker_1.mlWorker.extractEntities(batch);
...
async function extractEntitiesWithML(text) {
...
function maybeRefreshBaselines(now) {
    if (now - lastBaselineRefreshMs < BASELINE_REFRESH_MS)
        return;
...
function buildBaseTermCandidates(title) {
    ...
    for (const token of (0, analysis_constants_1.tokenize)(cleanTitle)) {
...
function recordTermCandidates(termCandidates, headline, now, blockedTerms) {
    let addedAny = false;
    for (const [term, meta] of termCandidates) {
        if (blockedTerms.has(term))
            continue;
        if (!meta.isEntity && term.length < MIN_TOKEN_LENGTH)
            continue;
...
function checkForSpikes(now, config, blockedTerms) {
    const spikes = [];
    for (const [term, record] of termFrequency) {
        if (blockedTerms.has(term))
            continue;
        const recentCount = record.timestamps.filter(ts => now - ts < ROLLING_WINDOW_MS).length;
        if (recentCount < config.minSpikeCount)
            continue;
        ...
        if (!isSpike)
            continue;
        if (now - record.lastSpikeAlertMs < SPIKE_COOLDOWN_MS)
            continue;
        const recentHeadlines = dedupeHeadlines(record.headlines.filter(headline => now - headline.ingestedAt <= ROLLING_WINDOW_MS));
        const uniqueSources = new Set(recentHeadlines.map(headline => headline.source)).size;
        if (uniqueSources < MIN_SPIKE_SOURCE_COUNT)
            continue;
...
function canRunAutoSummary(now) {
    while (autoSummaryRuns.length > 0 && now - autoSummaryRuns[0] > HOUR_MS) {
        autoSummaryRuns.shift();
    }
...
function isLikelyProperNoun(term, headlines) {
    if (term.includes(' ') && term.length > 5)
        return true;
    if (/^\d/.test(term))
        return true;
    const titles = headlines.slice(0, 8).map(h => h.title);
    const termRe = new RegExp(`\\b${(0, analysis_constants_1.escapeRegex)(term)}\\b`, 'gi');
    ...
    for (const title of titles) {
        for (const m of title.matchAll(termRe)) {
            ...
            if (idx === 0)
                continue;
            midSentenceCount++;
            if (/[A-Z]/.test(title[idx]))
                capitalizedCount++;
        }
    }
    if (midSentenceCount === 0) {
        return titles.some(t => {
            const allCaps = t.match(new RegExp(`\\b${(0, analysis_constants_1.escapeRegex)(term)}\\b`, 'gi'));
            return allCaps?.some(match => match === match.toUpperCase() && match.length >= 2);
        });
    }
    return capitalizedCount / midSentenceCount >= 0.5;
}
async function isSignificantTerm(term, headlines) {
    ...
    try {
        ...
        const entitiesPerTitle = await ml_worker_1.mlWorker.extractEntities(titles);
...
async function handleSpike(spike, config) {
    const termKey = toTermKey(spike.term);
    if (activeSpikeTerms.has(termKey))
        return;
    ...
    try {
        ...
        if (config.autoSummarize && headlines.length >= 2 && canRunAutoSummary(now)) {
            ...
            const summary = await (0, summarization_1.generateSummary)(headlines, undefined, `Breaking: "${spike.term}" mentioned ${spike.count}x in ${windowHours}h (${multiplierText})`);
            if (summary?.summary) {
        ...
        pushSignal({
            ...
            type: 'keyword_spike',
            title: (0, i18n_1.t)('alerts.trending', { term: spike.term, count: spike.count, hours: windowHours }),
            description,
            confidence,
            timestamp: new Date(),
            data: {
...
async function enrichWithMLEntities(headlines, ingestedAt) {
    if (headlines.length === 0 || !ml_worker_1.mlWorker.isAvailable)
        return;
    try {
        ...
        for (let i = 0; i < headlines.length; i += 1) {
            ...
            if (mlEntities.length === 0)
                continue;
            const termCandidates = new Map();
            for (const entity of mlEntities) {
                const termKey = toTermKey(entity);
                if (!termKey || pending.baseTermKeys.has(termKey))
                    continue;
                termCandidates.set(termKey, { display: entity, isEntity: true });
            }
...
function ingestHeadlines(headlines) {
    if (headlines.length === 0)
        return;
    const now = Date.now();
    const config = readConfig();
    const blockedTerms = getBlockedTermSet(config);
    const pendingMLEnrichment = [];
    for (const headline of headlines) {
...
function updateTrendingConfig(update) {
...
function suppressTrendingTerm(term) {
    const config = readConfig();
...
function unsuppressTrendingTerm(term) {
    const config = readConfig();
