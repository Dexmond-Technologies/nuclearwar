...
exports.checkWingbitsStatus = checkWingbitsStatus;
exports.getAircraftDetails = getAircraftDetails;
exports.getAircraftDetailsBatch = getAircraftDetailsBatch;
exports.analyzeAircraftDetails = analyzeAircraftDetails;
exports.enrichAircraft = enrichAircraft;
exports.getWingbitsStatus = getWingbitsStatus;
exports.clearWingbitsCache = clearWingbitsCache;
...
function sweepLocalCache(now = Date.now()) {
...
function createNegativeDetailsEntry(icao24) {
    return {
        icao24,
        registration: null,
 ...
 */
async function checkWingbitsStatus() {
    if (!(0, runtime_config_1.isFeatureAvailable)('wingbitsEnrichment'))
        return false;
    if (wingbitsConfigured !== null)
        return wingbitsConfigured;
    try {
        const resp = await client.getWingbitsStatus({});
        wingbitsConfigured = resp.configured;
        data_freshness_1.dataFreshness.setEnabled('wingbits', wingbitsConfigured);
        return wingbitsConfigured;
    }
    catch {
        wingbitsConfigured = false;
        data_freshness_1.dataFreshness.setEnabled('wingbits', false);
        return false;
    }
}
/**
 ...
 */
async function getAircraftDetailsBatch(icao24List) {
    if (!(0, runtime_config_1.isFeatureAvailable)('wingbitsEnrichment'))
        return new Map();
    ...
    try {
        ...
        if (results.size > 0) {
            data_freshness_1.dataFreshness.recordUpdate('wingbits', results.size);
        }
    }
    catch (error) {
        console.warn('[Wingbits] Batch fetch failed:', error);
        data_freshness_1.dataFreshness.recordError('wingbits', error instanceof Error ? error.message : 'Unknown error');
    }
    return results;
}
/**
 ...
 */
function analyzeAircraftDetails(details) {
    const result = {
...
async function enrichAircraft(icao24) {
    ...
    if (!details || !details.registration)
        return null;
