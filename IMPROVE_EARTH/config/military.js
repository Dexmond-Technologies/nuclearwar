...
exports.USNI_REGION_COORDINATES = exports.MILITARY_QUERY_REGIONS = exports.MILITARY_HOTSPOTS = exports.KNOWN_NAVAL_VESSELS = exports.MILITARY_VESSEL_PATTERNS = exports.MILITARY_HEX_RANGES = exports.MILITARY_AIRCRAFT_TYPES = exports.ALL_MILITARY_CALLSIGNS = exports.ADVERSARY_CALLSIGNS = exports.NATO_ALLIED_CALLSIGNS = exports.US_MILITARY_CALLSIGNS = void 0;
exports.normalizeUSNIRegion = normalizeUSNIRegion;
exports.getUSNIRegionCoords = getUSNIRegionCoords;
exports.getUSNIRegionApproxCoords = getUSNIRegionApproxCoords;
exports.identifyByCallsign = identifyByCallsign;
exports.identifyByAircraftType = identifyByAircraftType;
exports.isKnownMilitaryHex = isKnownMilitaryHex;
exports.getNearbyHotspot = getNearbyHotspot;
// US Military callsign patterns
exports.US_MILITARY_CALLSIGNS = [
    // USAF
...
exports.ALL_MILITARY_CALLSIGNS = [
    ...exports.US_MILITARY_CALLSIGNS,
    ...exports.NATO_ALLIED_CALLSIGNS,
    ...exports.ADVERSARY_CALLSIGNS,
];
 ...
 */
exports.MILITARY_AIRCRAFT_TYPES = {
    // Fighters
...
exports.MILITARY_VESSEL_PATTERNS = [
...
function normalizeUSNIRegion(regionText) {
    return regionText
        .replace(/^(In the|In|The)\s+/i, '')
        .replace(/\s+/g, ' ')
        .trim();
}
function getUSNIRegionCoords(regionText) {
    const normalized = normalizeUSNIRegion(regionText);
    if (exports.USNI_REGION_COORDINATES[normalized])
        return exports.USNI_REGION_COORDINATES[normalized];
    const lower = normalized.toLowerCase();
    for (const [key, coords] of Object.entries(exports.USNI_REGION_COORDINATES)) {
...
function getUSNIRegionApproxCoords(regionText) {
    const direct = getUSNIRegionCoords(regionText);
    if (direct)
        return direct;
    const normalized = normalizeUSNIRegion(regionText).toLowerCase();
    if (normalized.includes('eastern pacific'))
        return { lat: 18.0, lon: -125.0 };
    if (normalized.includes('western atlantic'))
        return { lat: 30.0, lon: -60.0 };
    if (normalized.includes('pacific'))
        return { lat: 15.0, lon: -150.0 };
    if (normalized.includes('atlantic'))
        return { lat: 30.0, lon: -40.0 };
    if (normalized.includes('indian'))
        return { lat: -5.0, lon: 75.0 };
    if (normalized.includes('mediterranean'))
        return { lat: 35.0, lon: 18.0 };
    if (normalized.includes('antarctic') || normalized.includes('southern'))
        return { lat: -70.0, lon: 20.0 };
    if (normalized.includes('arctic'))
        return { lat: 75.0, lon: 0.0 };
...
function identifyByCallsign(callsign, originCountry) {
    ...
    const preferred = [];
    if (origin === 'united kingdom' || origin === 'uk')
        preferred.push('rn', 'raf');
    if (origin === 'united states' || origin === 'usa')
        preferred.push('usn', 'usaf', 'usa', 'usmc');
    if (preferred.length > 0) {
        for (const pattern of exports.ALL_MILITARY_CALLSIGNS) {
            if (!preferred.includes(pattern.operator))
                continue;
 ...
 */
function identifyByAircraftType(typeCode) {
    const normalized = typeCode.toUpperCase().replace(/[^A-Z0-9]/g, '');
    return exports.MILITARY_AIRCRAFT_TYPES[normalized];
}
/**
...
function isKnownMilitaryHex(hexCode) {
    ...
    for (const range of exports.MILITARY_HEX_RANGES) {
...
function getNearbyHotspot(lat, lon) {
    for (const hotspot of exports.MILITARY_HOTSPOTS) {
