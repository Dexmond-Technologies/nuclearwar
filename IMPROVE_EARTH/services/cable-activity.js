...
exports.fetchCableActivity = fetchCableActivity;
const config_1 = require("@/config");
const service_client_1 = require("@/generated/client/worldmonitor/maritime/v1/service_client");
const maritimeClient = new service_client_1.MaritimeServiceClient('', { fetch: (...args) => globalThis.fetch(...args) });
...
function extractCableshipName(text) {
...
function findNearestCable(lat, lon) {
    ...
    for (const cable of config_1.UNDERSEA_CABLES) {
...
function determineShipStatus(text) {
...
function formatNgaDate(epochMs) {
    if (!epochMs)
        return '';
...
async function fetchCableActivity() {
    try {
        const response = await maritimeClient.listNavigationalWarnings({ area: '', pageSize: 0, cursor: '' });
        const warnings = response.warnings.map(protoToNgaWarning);
