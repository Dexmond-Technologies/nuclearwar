...
exports.POSITIVE_GDELT_TOPICS = exports.INTEL_TOPICS = void 0;
exports.getIntelTopics = getIntelTopics;
exports.fetchGdeltArticles = fetchGdeltArticles;
exports.fetchHotspotContext = fetchHotspotContext;
exports.fetchTopicIntelligence = fetchTopicIntelligence;
exports.fetchAllTopicIntelligence = fetchAllTopicIntelligence;
exports.formatArticleDate = formatArticleDate;
exports.extractDomain = extractDomain;
exports.fetchPositiveGdeltArticles = fetchPositiveGdeltArticles;
exports.fetchPositiveTopicIntelligence = fetchPositiveTopicIntelligence;
exports.fetchAllPositiveTopicIntelligence = fetchAllPositiveTopicIntelligence;
const i18n_1 = require("@/services/i18n");
const service_client_1 = require("@/generated/client/worldmonitor/intelligence/v1/service_client");
const utils_1 = require("@/utils");
exports.INTEL_TOPICS = [
    {
        id: 'military',
...
function getIntelTopics() {
    return exports.INTEL_TOPICS.map(topic => ({
        ...topic,
        name: (0, i18n_1.t)(`intel.topics.${topic.id}.name`),
        description: (0, i18n_1.t)(`intel.topics.${topic.id}.description`),
    }));
}
// ---- Sebuf client ----
const client = new service_client_1.IntelligenceServiceClient('', { fetch: (...args) => globalThis.fetch(...args) });
const gdeltBreaker = (0, utils_1.createCircuitBreaker)({ name: 'GDELT Intelligence', cacheTtlMs: 5 * 60 * 1000, persistCache: true });
const emptyGdeltFallback = { articles: [], query: '', error: '' };
const CACHE_TTL = 5 * 60 * 1000;
const articleCache = new Map();
/** Map proto GdeltArticle (all required strings) to service GdeltArticle (optional fields) */
function toGdeltArticle(a) {
    return {
        title: a.title,
        url: a.url,
        source: a.source,
...
async function fetchGdeltArticles(query, maxrecords = 10, timespan = '24h') {
...
async function fetchHotspotContext(hotspot) {
    const query = hotspot.keywords.slice(0, 5).join(' OR ');
    return fetchGdeltArticles(query, 8, '48h');
}
async function fetchTopicIntelligence(topic) {
...
async function fetchAllTopicIntelligence() {
    const results = await Promise.allSettled(exports.INTEL_TOPICS.map(topic => fetchTopicIntelligence(topic)));
    return results
...
async function fetchPositiveGdeltArticles(query, toneFilter = 'tone>5', sort = 'ToneDesc', maxrecords = 15, timespan = '72h') {
...
async function fetchPositiveTopicIntelligence(topic) {
    const articles = await fetchPositiveGdeltArticles(topic.query);
    return { topic, articles, fetchedAt: new Date() };
}
async function fetchAllPositiveTopicIntelligence() {
    const results = await Promise.allSettled(exports.POSITIVE_GDELT_TOPICS.map(topic => fetchPositiveTopicIntelligence(topic)));
    return results
        .filter((r) => r.status === 'fulfilled')
        .map(r => r.value);
}
