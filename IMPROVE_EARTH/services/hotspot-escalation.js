...
exports.setCIIGetter = setCIIGetter;
exports.setGeoAlertGetter = setGeoAlertGetter;
exports.calculateDynamicScore = calculateDynamicScore;
exports.getHotspotEscalation = getHotspotEscalation;
exports.getAllEscalationScores = getAllEscalationScores;
exports.shouldEmitSignal = shouldEmitSignal;
exports.markSignalEmitted = markSignalEmitted;
exports.countMilitaryNearHotspot = countMilitaryNearHotspot;
exports.setMilitaryData = setMilitaryData;
exports.updateHotspotEscalation = updateHotspotEscalation;
exports.getEscalationChange24h = getEscalationChange24h;
exports.clearEscalationData = clearEscalationData;
...
function setCIIGetter(fn) {
    ciiGetter = fn;
}
function setGeoAlertGetter(fn) {
    geoAlertGetter = fn;
}
...
function getCIIForHotspot(hotspotId) {
    if (!ciiGetter)
        return null;
    const countryCodes = (0, countries_1.getHotspotCountries)(hotspotId);
...
function calculateDynamicScore(hotspotId, inputs) {
    const hotspot = geo_1.INTEL_HOTSPOTS.find(h => h.id === hotspotId);
    if (!hotspot) {
...
function getHotspotEscalation(hotspotId) {
    return scores.get(hotspotId) ?? null;
}
function getAllEscalationScores() {
    return Array.from(scores.values());
}
function shouldEmitSignal(hotspotId, oldScore, newScore) {
...
function markSignalEmitted(hotspotId) {
...
function setMilitaryData(flights, vessels) {
    militaryData = { flights, vessels };
}
function updateHotspotEscalation(hotspotId, newsMatches, hasBreaking, newsVelocity) {
    const hotspot = geo_1.INTEL_HOTSPOTS.find(h => h.id === hotspotId);
    if (!hotspot)
        return null;
...
function getEscalationChange24h(hotspotId) {
