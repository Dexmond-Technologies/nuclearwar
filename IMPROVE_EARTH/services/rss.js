...
exports.getFeedFailures = getFeedFailures;
exports.fetchFeed = fetchFeed;
exports.fetchCategoryFeeds = fetchCategoryFeeds;
...
const trending_keywords_1 = require("./trending-keywords");
...
const AI_CLASSIFY_MAX_PER_WINDOW = config_1.SITE_VARIANT === 'finance' ? 40 : config_1.SITE_VARIANT === 'tech' ? 60 : 80;
const AI_CLASSIFY_MAX_PER_FEED = config_1.SITE_VARIANT === 'finance' ? 2 : config_1.SITE_VARIANT === 'tech' ? 2 : 3;
const aiRecentlyQueued = new Map();
const aiDispatches = [];
function toSerializable(items) {
    return items.map(item => ({ ...item, pubDate: item.pubDate.toISOString() }));
}
function fromSerializable(items) {
    return items.map(item => ({ ...item, pubDate: new Date(item.pubDate) }));
}
function getFeedScope(feedName, lang) {
    return `${feedName}${FEED_SCOPE_SEPARATOR}${lang}`;
}
function parseFeedScope(feedScope) {
    const splitIndex = feedScope.lastIndexOf(FEED_SCOPE_SEPARATOR);
    if (splitIndex === -1)
        return { feedName: feedScope, lang: 'en' };
    return {
        feedName: feedScope.slice(0, splitIndex),
        lang: feedScope.slice(splitIndex + FEED_SCOPE_SEPARATOR.length),
    };
}
function getPersistentFeedKey(feedScope) {
    return `feed:${feedScope}`;
}
async function readPersistentFeed(key) {
    const entry = await (0, persistent_cache_1.getPersistentCache)(key);
    if (!entry?.data?.length)
        return null;
    return fromSerializable(entry.data);
}
async function loadPersistentFeed(feedScope) {
...
function isFeedOnCooldown(feedScope) {
...
function recordFeedFailure(feedScope) {
...
function recordFeedSuccess(feedScope) {
    feedFailures.delete(feedScope);
}
function getFeedFailures() {
    const currentLang = (0, i18n_1.getCurrentLanguage)();
    const currentLangFailures = new Map();
...
function toAiKey(title) {
    return title.trim().toLowerCase().replace(/\s+/g, ' ');
}
function canQueueAiClassification(title) {
    const now = Date.now();
...
async function fetchFeed(feed) {
    ...
    try {
        ...
        if (!url)
            throw new Error(`No URL found for feed ${feed.name}`);
        const response = await (0, utils_1.fetchWithProxy)(url);
        if (!response.ok)
        ...
        const parsed = Array.from(items)
            ...
            const threat = (0, threat_classifier_1.classifyByKeyword)(title, config_1.SITE_VARIANT);
            const isAlert = threat.level === 'critical' || threat.level === 'high';
            const geoMatches = (0, geo_hub_index_1.inferGeoHubsFromTitle)(title);
            ...
            return {
                ...
                lang: feed.lang,
                ...(config_1.SITE_VARIANT === 'happy' && { imageUrl: extractImageUrl(item) }),
            };
        });
        feedCache.set(feedScope, { items: parsed, timestamp: Date.now() });
        void (0, persistent_cache_1.setPersistentCache)(getPersistentFeedKey(feedScope), toSerializable(parsed));
        recordFeedSuccess(feedScope);
        (0, trending_keywords_1.ingestHeadlines)(parsed.map(item => ({
            title: item.title,
        ...
        for (const item of aiCandidates) {
            if (!canQueueAiClassification(item.title))
                continue;
            (0, threat_classifier_1.classifyWithAI)(item.title, config_1.SITE_VARIANT).then((aiResult) => {
...
async function fetchCategoryFeeds(feeds, options = {}) {
    const topLimit = 20;
    ...
    const batches = (0, utils_1.chunkArray)(filteredFeeds, batchSize);
    const topItems = [];
    let totalItems = 0;
    const ensureSortedDescending = () => [...topItems].sort((a, b) => b.pubDate.getTime() - a.pubDate.getTime());
    const insertTopItem = (item) => {
        totalItems += 1;
    ...
    if (totalItems > 0) {
        data_freshness_1.dataFreshness.recordUpdate('rss', totalItems);
    }
