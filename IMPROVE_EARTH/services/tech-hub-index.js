...
exports.inferHubsFromTitle = inferHubsFromTitle;
exports.getHubById = getHubById;
exports.getAllHubs = getAllHubs;
exports.getHubsByTier = getHubsByTier;
const startup_ecosystems_1 = require("@/config/startup-ecosystems");
const tech_companies_1 = require("@/config/tech-companies");
const tech_geo_1 = require("@/config/tech-geo");
const keyword_match_1 = require("@/utils/keyword-match");
let cachedIndex = null;
function normalizeTier(tier) {
    if (!tier)
        return 'emerging';
    if (tier === 'tier1' || tier === 'mega')
        return 'mega';
    if (tier === 'tier2' || tier === 'major')
        return 'major';
    return 'emerging';
}
function buildTechHubIndex() {
    if (cachedIndex)
        return cachedIndex;
    const hubs = new Map();
    const byKeyword = new Map();
    const addKeyword = (keyword, hubId) => {
    ...
    for (const eco of startup_ecosystems_1.STARTUP_ECOSYSTEMS) {
        const hub = {
            id: eco.id,
            name: eco.name,
            city: eco.city,
    ...
    for (const company of tech_companies_1.TECH_COMPANIES) {
    ...
    for (const sh of tech_geo_1.STARTUP_HUBS) {
...
function inferHubsFromTitle(title) {
    const index = buildTechHubIndex();
    const matches = [];
    const tokens = (0, keyword_match_1.tokenizeForMatch)(title);
    const seenHubs = new Set();
    for (const [keyword, hubIds] of index.byKeyword) {
        if (keyword.length < 3)
            continue;
        if ((0, keyword_match_1.matchKeyword)(tokens, keyword)) {
            for (const hubId of hubIds) {
                if (seenHubs.has(hubId))
                    continue;
                seenHubs.add(hubId);
