...
// Auth Button 3-Second Wait Initialization Hook
let authWaitVal = 3;

const authWaitInterval = setInterval(() => {
    authWaitVal--;
    const elWaitVal = document.getElementById('auth-countdown-val');
    if(elWaitVal) elWaitVal.innerText = authWaitVal;
    
    if (authWaitVal <= 0) {
        clearInterval(authWaitInterval);
        const container = document.getElementById('auth-countdown-container');
        const buttons = document.getElementById('auth-buttons-container');
...
function initD3XCaves() {
  ...
  coords.forEach(cave => {
    ...
    const rockMat = new THREE.MeshStandardMaterial({ color: 0x555555, roughness: 0.2, metalness: 0.8, transparent: true, opacity: 0.9 });
    ...
    // Glowing core
    const coreGeo = new THREE.DodecahedronGeometry(0.045, 0);
    const coreMat = new THREE.MeshBasicMaterial({ color: 0xff00ff });
    const core = new THREE.Mesh(coreGeo, coreMat);
    core.userData.isD3XCore = true; // Tag for animation loop
    group.add(core);

    // Text Label Maker ("D3X" directly on the rocks)
    const canvas = document.createElement('canvas');
    canvas.width = 128; canvas.height = 128;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = 'rgba(255, 255, 255, 0.9)'; // Bright white to contrast with rainbow rocks
    ctx.font = 'bold 40px monospace';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText("D3X", 64, 64);
    const tex = new THREE.CanvasTexture(canvas);
    const spriteMat = new THREE.SpriteMaterial({ map: tex, transparent: true, depthTest: false });
    const sprite = new THREE.Sprite(spriteMat);
    sprite.scale.set(0.2, 0.2, 1);
    sprite.position.set(0, 0, 0); // Centered exactly on the rock core
    group.add(sprite);
...
function initDatacenters() {
    ...
    dcData.forEach(dc => {
        ...
        if (isD3XExchange) {
            ...
            // Add an inner glowing aura
            const auraGeo = new THREE.SphereGeometry(0.035, 32, 32);
            const auraMat = new THREE.MeshBasicMaterial({ color: 0xffaa00, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending });
            const aura = new THREE.Mesh(auraGeo, auraMat);
            mesh.add(aura);
            ...
            if (isD3XExchange) {
                ...
                // MOVED TESLA TOWERS TO TOP OF THE GLOBE
                const npLat = 90; // Exactly on top of the globe
                const npLon = 0;
...
function initDatacenterNetwork() {
    ...
    // Yellow tint for the lines, fully visible
    const material = new THREE.LineBasicMaterial({ color: 0xffff00, transparent: true, opacity: 0.6, linewidth: 3 });
    ...
    const pulseMat = new THREE.SpriteMaterial({ map: pulseTex, color: 0xffffff, transparent: true, blending: THREE.AdditiveBlending });
...
function initGlobalTradeNetwork() {
  ...
  routes.forEach(rt => {
    // Inject 4 vehicles traversing each global route (we added 160 routes, 4x bidirectional = massive density)
    for(let i=0; i<4; i++) {
...
async function initWebcams() {
  try {
    ...
    data.result.webcams.forEach(cam => {
      ...
      const sprite = new THREE.Sprite(mat);
      sprite.scale.set(0.05, 0.05, 1); // Moderate yellow dot
      
      const pos = ll2v(parseFloat(cam.latitude), parseFloat(cam.longitude), GLOBE_R + 0.05);
      sprite.position.copy(pos);
...
window.showCmdHelpModal = function() {
    ...
    contentBox.innerHTML = `
      ...
      <div style="display:flex; gap:30px; margin-bottom:30px; flex-wrap:wrap;">
        ...
        <div style="flex:1; min-width:300px; background:rgba(255,51,102,0.05); padding:25px; border-radius:10px; border-left:5px solid #ff3366;">
          <span style="display:block; font-size:24px; color:#ff3366; margin-bottom:15px; font-weight:bold;">` + ((idx === 0 || idx === 1 || idx === 2 || idx === 3) ? "2. Active Commander Access" : "2. Restricted Interactivity") + `</span>
          ` + skinSpecificInteractivity + `
        </div>
      </div>
  ...
  // Specifically removing for Live (0), Red (1), Green (2), and Cyber (3)
...
function handleGlobeClick(e) {
  ...
  // 1. Check Webcams and Boats first if Red Skin is active
  if (window.isRedSkin && (webcamSprites.length > 0 || boatMeshes.length > 0)) {
  ...
  if (window.currentSkinIndex === 0 && window.issGroup && window.issGroup.visible) {
  ...
  // Intercept clicks on Boat Markers for Naval War Mini-Game
  if (window.isRedSkin && boatMarkers && boatMarkers.length > 0) {
    const boatHits = ray.intersectObjects(boatMarkers, false);
    if (boatHits.length > 0) {
      if (typeof window.openNavalWar === 'function') {
        window.openNavalWar(boatHits[0].object);
      }
      return; // Intercept and stop further map propagation
    }
  }
  ...
  if (window.currentSkinIndex === 3 && typeof d3xCaveMarkers !== 'undefined' && d3xCaveMarkers.length > 0) {
    ...
    if (caveHits.length > 0) {
      ...
      openD3XMiningCave(caveName);
      return;
    }
  }

  // Intercept clicks on Red Satellites for Satellite Mini-Game
  if (window.isRedSkin && typeof redSatMarkers !== 'undefined' && redSatMarkers.length > 0) {
    const satHits = ray.intersectObjects(redSatMarkers, true);
    if (satHits.length > 0) {
      if (typeof window.openSatelliteWar === 'function') {
        window.openSatelliteWar(satHits[0].object);
      }
      return;
    }
  }
  
  // Intercept clicks on Airplane Markers for Shooter Mini-Game
  if (window.isRedSkin && window.planeMarkers && window.planeMarkers.length > 0) {
    const planeHits = ray.intersectObjects(window.planeMarkers, true);
    if (planeHits.length > 0) {
      if (typeof window.openShooterGame === 'function') {
        window.openShooterGame(planeHits[0].object);
      }
      return; // Intercept and stop further map propagation
    }
  }
  ...
  if (window.currentSkinIndex === 4 && window.datacentersGroup && window.datacentersGroup.visible && window.datacentersGroup.children.length > 0) {
    ...
    if (dcHits.length > 0) {
      ...
      if (dcData && dcData.type === 'staking_tower') {
          ...
          document.getElementById('staking-tower-balance').textContent = window.localD3XBalance ? window.localD3XBalance.toFixed(2) : '100';
...
window.greenMarket = {
  commodities: {
    // METALS (scaled down from Metric Ton / Global Quote to per-unit/kg for gameplay)
    'Copper': { price: 12.98, vol: 0.02, shares: 0, history: [12.98] }, // 12986 / 1000
    'Lithium': { price: 68.97, vol: 0.05, shares: 0, history: [68.97] }, // ETF proxy
    'Rare Earth Elements': { price: 92.53, vol: 0.08, shares: 0, history: [92.53] },
    'Iron ore': { price: 14.97, vol: 0.01, shares: 0, history: [14.97] }, // ETF proxy
    'Gold': { price: 473.51, vol: 0.015, shares: 0, history: [473.51] }, // ETF proxy
    'Nickel': { price: 3.02, vol: 0.03, shares: 0, history: [3.02] }, // ETF proxy
    'Tin': { price: 71.85, vol: 0.04, shares: 0, history: [71.85] },
    'Aluminum (Bauxite)': { price: 3.13, vol: 0.02, shares: 0, history: [3.13] }, // 3133 / 1000
    'Aluminum': { price: 3.13, vol: 0.02, shares: 0, history: [3.13] }, // 3133 / 1000
    'Cobalt': { price: 71.85, vol: 0.06, shares: 0, history: [71.85] },
    
    // NATURAL RESOURCES
    'Lumber (Wood)': { price: 5.40, vol: 0.015, shares: 0, history: [5.40] },
    'Freshwater': { price: 0.85, vol: 0.005, shares: 0, history: [0.85] },
    'Natural Gas': { price: 2.15, vol: 0.04, shares: 0, history: [2.15] },
    'Crude Oil': { price: 82.40, vol: 0.03, shares: 0, history: [82.40] },
    'Wheat': { price: 6.20, vol: 0.02, shares: 0, history: [6.20] }
  },
  lastUpdate: Date.now()
};
...
function updateGreenMarket() {
  ...
  Object.keys(window.greenMarket.commodities).forEach(k => {
    ...
    // Process Volatility Random Walk
    let change = 1 + (Math.random() * c.vol * 2 - c.vol);
    // 5% chance of Rainclaude market manipulation (pump or dump)
    ...
    let shareVal = c.price / 100; // Value per share in MIL
    ...
    if (c.shares > 0) {
      holdingsHtml += `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px; background:rgba(0,255,136,0.1); padding:4px; border-radius:3px; border:1px solid rgba(0,255,136,0.2); font-size:9px;">
          ...
          <span style="font-family:'Orbitron', monospace; font-size:10px; font-weight:bold; text-align:right;">${trend} $${shareVal.toFixed(2)}M</span>
        </div>
      `;
    }
    ...
    if (priceEl) {
      priceEl.innerHTML = `${trend} $${shareVal.toFixed(2)} MIL <span style="font-size:10px; color:#aaa; font-weight:normal;">/ SHARE</span>`;
    }
    if (sellBtn) {
  ...
  if (networthEl) {
    networthEl.innerText = '$' + totalWorth.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2}) + ' M';
...
window.tradeGreenMetal = function(metalName, amount) {
  ...
  if (window.localD3XBalance === undefined) window.localD3XBalance = 100;

  const cost = c.price / 100; // MIL cost translates 1:1 to D3X here for simplicity
  const totalCost = Math.round(cost * Math.abs(amount) * 100) / 100;

  if (amount > 0) {
    if (window.localD3XBalance >= totalCost) {
      window.localD3XBalance -= totalCost;
      c.shares += amount;
      if (typeof setLog === 'function') setLog(`📈 ACQUIRED: ${amount}x ${metalName} stake for ${totalCost} D3X.`);
    } else {
      if (typeof setLog === 'function') setLog(`❌ INSUFFICIENT FUNDS: ${totalCost} D3X needed for ${metalName} stake.`);
    }
  } else if (amount < 0) {
    if (c.shares >= Math.abs(amount)) {
      c.shares += amount; 
      window.localD3XBalance += totalCost;
      if (typeof setLog === 'function') setLog(`📉 LIQUIDATED: ${Math.abs(amount)}x ${metalName} stake for a ${totalCost} D3X return.`);
    } else {
      if (typeof setLog === 'function') setLog(`❌ NO STAKE: You do not own shares of ${metalName} to sell.`);
    }
  }
  updateGreenMarket();
  if (typeof updateDashBoard === 'function') updateDashBoard(); // Refresh user budget graphic immediately
};

updateGreenMarket(); // Initial render to prevent 2-3s delay on script load
setInterval(updateGreenMarket, 3000);
...
window.showGMarketWallet = function() {
  ...
  if (!modal) {
      ...
      modal.style.backgroundColor = 'rgba(0,18,8,0.95)';
      modal.style.border = '2px solid #0f8';
      modal.style.boxShadow = '0 0 25px rgba(0,255,136,0.4)';
      modal.style.zIndex = '99999';
      modal.style.display = 'flex';
      modal.style.flexDirection = 'column';
      ...
      contentBox.style.maxHeight = '70vh';
  ...
  let listHtml = `<table style="width:100%; border-collapse:collapse; font-size:13px;">
    <tr style="border-bottom:1px dashed #0f8; color:#0f8;">
      ...
      <th style="padding:10px 8px; text-align:right;">PRICE / SHARE</th>
      <th style="padding:10px 8px; text-align:right;">OWNED</th>
      <th style="padding:10px 8px; text-align:right;">TOTAL VALUE</th>
  ...
  Object.keys(window.greenMarket.commodities).forEach(k => {
    ...
    listHtml += `
      <tr style="border-bottom:1px solid rgba(0,255,136,0.15);">
        ...
        <td style="padding:10px 8px; text-align:right; font-family:'Courier New', monospace; font-weight:bold;">${trend} $${shareVal.toFixed(2)}M</td>
        <td style="padding:10px 8px; text-align:right; font-weight:bold; color:${c.shares > 0 ? '#0f8' : '#aaa'};">${c.shares}</td>
        <td style="padding:10px 8px; text-align:right; font-family:'Courier New', monospace; color:${c.shares > 0 ? '#0f8' : '#aaa'};">$${holdVal.toFixed(2)}M</td>
  ...
  listHtml += `
  <div style="display:flex; justify-content:space-between; margin-top:20px; padding-top:15px; border-top:1px dashed #0f8;">
    <div style="font-size:14px; font-weight:bold; color:#aaa; font-family:'Courier New', monospace;">
      AVAILABLE D3X WALLET BALANCE: <span style="color:#fff;">${window.localD3XBalance.toFixed(2)} D3X</span>
    </div>
    <div style="text-align:right; font-size:16px; font-weight:bold; color:#fff;">
      NET WORTH (HOLDINGS): <span style="color:#0f8; font-family:'Courier New', monospace;">$${totalVal.toFixed(2)} M</span>
    </div>
  </div>`;
...
function showGreenSkinResourceModal(iso) {
  ...
  if (METALS_DATA[cName]) {
    metalsHtml = `<div style="margin-top:20px; border-top: 1px solid rgba(255,255,255,0.2); padding-top: 15px;">
  ...
  const content = `
    ...
    <div style="padding-right: 5px;">
      <div class="strat-item">
        <div class="strat-item-title" style="color:#fa0;">🛢 Oil (Petroleum)</div>
        <div class="strat-item-desc">State monopolies or direct government control over production (e.g., National Oil Companies).</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#0f8;">💨 Natural Gas</div>
        <div class="strat-item-desc">State-controlled extraction, reserves owned by the nation.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#aaa;">⛏ Coal</div>
        <div class="strat-item-desc">Government-dominated production or heavily regulated public land leasing.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#0f0;">☢ Uranium</div>
        <div class="strat-item-desc">Heavily regulated and state-controlled for strategic/security purposes.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#f88;">🌿 Phosphate Rock</div>
        <div class="strat-item-desc">Key fertilizer component; production is strictly state-influenced or owned.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#ffc;">🌾 Potash</div>
        <div class="strat-item-desc">Essential agricultural resource; governed by state leases and public resource management.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#ddf;">🧂 Salt</div>
        <div class="strat-item-desc">State-produced resource or regulated government monopoly.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#ccc;">🧱 Limestone</div>
        <div class="strat-item-desc">State-leased or directly controlled on public lands for construction.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#eda;">🏠 Gypsum</div>
        <div class="strat-item-desc">Managed through state building materials oversight and leasing.</div>
      </div>
      <div class="strat-item">
        <div class="strat-item-title" style="color:#08f;">💧 Water (Freshwater)</div>
        <div class="strat-item-desc">Governments strictly control extraction, distribution, and ownership.</div>
      </div>
      ${metalsHtml}
    </div>
  `;
  ...
  ws.onmessage=e=>{
    ...
    switch(msg.type){
      ...
      case 'ai_combat_log': {
        ...
        if (d0Hp && msg.gemini_hp !== undefined) d0Hp.textContent = msg.gemini_hp.toLocaleString();
        if (d1Hp && msg.claude_hp !== undefined) d1Hp.textContent = msg.claude_hp.toLocaleString();
...
function joinGlobalWar() {
  ...
  const walletDisp = document.getElementById('connected-wallet-display');
  const reinvestBtn = document.getElementById('btn-reinvest-d3x');
  if (walletDisp && window.pendingSolanaAddress) {
    const addr = window.pendingSolanaAddress;
    walletDisp.innerHTML = '🛡️ ' + addr.substring(0,4) + '...' + addr.substring(addr.length-4);
...
function reinvestD3X() {
    if (ws && ws.readyState === WebSocket.OPEN && myName) {
        ws.send(JSON.stringify({ type: 'reinvest_earnings', callsign: myName }));
        if (typeof setLog === 'function') setLog(`🔄 Initiating D3X Reinvestment Protocol...`);
    } else {
        if (typeof setLog === 'function') setLog(`⚠️ NETWORK ERROR: Cannot reinvest right now.`);
    }
}
...
function closeOnboarding() {
  ...
  if (typeof window.toggleSoundtrack === 'function' && typeof isSoundtrackOn !== 'undefined' && !isSoundtrackOn) {
...
function spawnAmbients() {
  ...
  for(let i=0; i<300; i++) { // Doubled from 150 to 300
    let currentMat = matSat; // 40% white (neutral)
    if (i < 90) { // 30% red (rainclaude)
      currentMat = matSatRed;
    } else if (i < 180) { // 30% blue (humanity)
...
function animate() {
  ...
  if (window.currentSkinIndex && window.currentSkinIndex > 0) {
    // 30 second cycle for laser scans
    window.lastLaserTime = window.lastLaserTime || 0;
    if (time3 - window.lastLaserTime > 30) {
      window.lastLaserTime = time3;
      // Turn off all existing lasers
      redSats.forEach(s => { if(s.laser) s.laser.visible = false; });
      // Pick 2 random satellites to fire
      if (redSats.length > 1) {
        let idx1 = Math.floor(Math.random() * redSats.length);
        let idx2 = Math.floor(Math.random() * redSats.length);
        while(idx1 === idx2) idx2 = Math.floor(Math.random() * redSats.length);
        
        // Dynamically color the laser based on the current skin color
        const skinColor = SKINS[window.currentSkinIndex].color;
        
        if(redSats[idx1].laser) {
          redSats[idx1].laser.material.color.setHex(skinColor);
          redSats[idx1].laser.visible = true;
        }
        if(redSats[idx2].laser) {
          redSats[idx2].laser.material.color.setHex(skinColor);
          redSats[idx2].laser.visible = true;
        }
        
        // Turn them off after 2 seconds
        setTimeout(() => {
          if(redSats[idx1] && redSats[idx1].laser) redSats[idx1].laser.visible = false;
          if(redSats[idx2] && redSats[idx2].laser) redSats[idx2].laser.visible = false;
        }, 2000);
      }
    }
    ...
    if (window.planeData) {
      for(let i=0; i<window.planeData.length; i++) {
        ...
        if (p && p.pivot) {
           p.pivot.rotateOnAxis(p.axis, p.speed * ((dt*60)||1));
           // Randomly drop bombs from planes
           if (window.isRedSkin && Math.random() < 0.005) {
               // Calculate world position of the plane
    ...
    // Randomly spawn boat rockets
    if (window.isRedSkin && typeof boatMarkers !== 'undefined' && boatMarkers.length > 1 && Math.random() < 0.02) {
        // Pick random shooter
    ...
    for (let i = redWarfareProjectiles.length - 1; i >= 0; i--) {
        ...
        if (proj.progress >= 1.0) {
            ...
            const expMat = new THREE.SpriteMaterial({ 
                map: new THREE.CanvasTexture(createGlowCanvas(proj.type === 'bomb' ? '#ff5500' : '#ffff00')), 
          ...
          window.npLightningGroup.children.forEach(bridge => {
              ...
              if (bridge.userData.timer <= 0) {
                  ...
                  if (allNodes.length > 0) {
                      ...
                      bridge.userData.timer = 4.0 + Math.random() * 4.0; // Hold connection for 4-8 seconds
                  }
              }
              ...
              if (bridge.userData.targetNode) {
                  ...
                  bridge.children.forEach(braid => {
                      ...
                      for (let i = 0; i <= segments; i++) {
                          ...
                          const r = braid.userData.radius * envelope * pulse;
  ...
  for(let i=fakeJets3D.length-1;i>=0;i--){
    ...
    fj.group.visible = !(_skin && (_skin.name === 'BLK' || _skin.name === 'GRN'));
...
function openDrawer(id, titleHtml, bodyHtml) {
  ...
  if (id === 'arms-ind') drawer.classList.add('centered-mode');
  drawer.id = 'drawer-' + id;
  drawer.innerHTML = `
...
function fetchAndRenderBoats() {
  ...
  const data = [];
  for (let loop = 0; loop < 2; loop++) { // Double from 10 to 20
      baseData.forEach(b => {
          data.push({
...
window.navalWarState = { active: false, humanHp: 17, aiHp: 17, boatMesh: null, humanGrid: [], aiGrid: [] };

function generateBattleshipGrid() {
    ...
    for(let size of sizes) {
        ...
        while(!placed) {
            ...
            if (dir === 'H' && c + size <= 10) {
                let clear = true;
                for(let i=0; i<size; i++) if(grid[r][c+i] !== 0) clear = false;
                if(clear) {
                    for(let i=0; i<size; i++) grid[r][c+i] = 1;
                    placed = true;
                }
            } else if (dir === 'V' && r + size <= 10) {
                let clear = true;
                for(let i=0; i<size; i++) if(grid[r+i][c] !== 0) clear = false;
                if(clear) {
                    for(let i=0; i<size; i++) grid[r+i][c] = 1;
                    placed = true;
                }
            }
        }
...
window.openNavalWar = function(mesh) {
  if (!window.isRedSkin) return;
  SFX.playClick();
  window.navalWarState = { 
     active: true, humanHp: 17, aiHp: 17, boatMesh: mesh,
     humanGrid: generateBattleshipGrid(),
     aiGrid: generateBattleshipGrid()
  };
  
  document.getElementById('naval-hp-human').textContent = '17';
  document.getElementById('naval-bar-human').style.width = '100%';
  document.getElementById('naval-hp-ai').textContent = '17';
  document.getElementById('naval-bar-ai').style.width = '100%';
  
  const log = document.getElementById('naval-log');
  log.innerHTML = '<div style="opacity:0.7;">Sonar contact acquired. Awaiting firing coordinates...</div>';
  
  window.renderNavalGrids();
  document.getElementById('naval-modal').classList.add('show');
};

window.renderNavalGrids = function() {
    const pGrid = document.getElementById('human-grid');
    const aGrid = document.getElementById('ai-grid');
    ...
    for(let r=0; r<10; r++) {
        for(let c=0; c<10; c++) {
            ...
            cellH.style.border = '1px solid rgba(0,170,255,0.3)';
            cellH.style.cursor = 'default';
            let valH = window.navalWarState.humanGrid[r][c];
            if (valH === 0) cellH.style.background = 'transparent'; // empty water
            else if (valH === 1) cellH.style.background = 'rgba(0,170,255,0.5)'; // ship
            else if (valH === 2) cellH.style.background = '#f55'; // hit
            else if (valH === 3) cellH.style.background = 'rgba(255,255,255,0.3)'; // miss
            pGrid.appendChild(cellH);
            
            // AI cell
            const cellA = document.createElement('div');
            cellA.style.border = '1px solid rgba(255,85,85,0.3)';
            cellA.style.cursor = 'pointer';
            let valA = window.navalWarState.aiGrid[r][c];
            
            if (valA === 0 || valA === 1) {
                cellA.style.background = 'transparent'; // hid AI ships
                cellA.onmouseover = function() { cellA.style.background = 'rgba(255,85,85,0.2)'; };
                cellA.onmouseout = function() { cellA.style.background = 'transparent'; };
                cellA.onclick = function() { window.fireAtAI(r, c); };
            } else if (valA === 2) {
                cellA.style.background = '#0f8'; // Player hit AI
                cellA.style.cursor = 'default';
            } else if (valA === 3) {
                cellA.style.background = 'rgba(255,255,255,0.3)'; // miss
                cellA.style.cursor = 'default';
            }
            aGrid.appendChild(cellA);
        }
    }
};

window.fireAtAI = function(r, c) {
    if (!window.navalWarState.active) return;
    
    let aGrid = window.navalWarState.aiGrid;
    let hit = (aGrid[r][c] === 1);
    aGrid[r][c] = hit ? 2 : 3;
    ...
    if (hit) {
        ...
        window.navalWarState.aiHp--;
        log.innerHTML += `<div><span style="color:#0f8">DIRECT HIT! Rainclaude vessel damaged at [${r},${c}].</span></div>`;
    } else {
        SFX.playClick();
        log.innerHTML += `<div><span style="color:#aaa">Miss! Torpedo detonated in open water at [${r},${c}].</span></div>`;
    }
    
    // Update AI HP bar
    document.getElementById('naval-hp-ai').textContent = window.navalWarState.aiHp;
    document.getElementById('naval-bar-ai').style.width = ((window.navalWarState.aiHp / 17) * 100) + '%';
    log.scrollTop = log.scrollHeight;
    
    window.renderNavalGrids();
    
    // Check win
    if (window.navalWarState.aiHp <= 0) {
        window.endNavalWar('human');
        return;
    }
    
    // AI Turn (Disable player input briefly by disabling active flag)
    window.navalWarState.active = false;
    setTimeout(() => {
        let pGrid = window.navalWarState.humanGrid;
        
        // simple random AI
        let aiR, aiC;
        do {
            aiR = Math.floor(Math.random() * 10);
            aiC = Math.floor(Math.random() * 10);
        } while(pGrid[aiR][aiC] === 2 || pGrid[aiR][aiC] === 3);
        
        let aiHit = (pGrid[aiR][aiC] === 1);
        pGrid[aiR][aiC] = aiHit ? 2 : 3;
        
        if (aiHit) {
             if(typeof SFX.playExplosion === 'function') SFX.playExplosion();
             else SFX.playSiren();
             window.navalWarState.humanHp--;
             log.innerHTML += `<div><span style="color:#f55">WARNING! Human vessel hit at [${aiR},${aiC}]!</span></div>`;
        } else {
             SFX.playClick();
             log.innerHTML += `<div><span style="color:#aaa">Rainclaude missed at [${aiR},${aiC}].</span></div>`;
        }
        
        document.getElementById('naval-hp-human').textContent = window.navalWarState.humanHp;
        document.getElementById('naval-bar-human').style.width = ((window.navalWarState.humanHp / 17) * 100) + '%';
        log.scrollTop = log.scrollHeight;
        
        window.renderNavalGrids();
        
        if (window.navalWarState.humanHp <= 0) {
            window.endNavalWar('ai');
        } else {
            window.navalWarState.active = true; // allow player to shoot again
        }
    }, 600);
}

window.endNavalWar = function(winner) {
    window.navalWarState.active = false;
    const log = document.getElementById('naval-log');
    if (winner === 'human') {
      log.innerHTML += `<div style="color:#0f8; font-weight:bold; font-size:14px; margin-top:10px;">TACTICAL VICTORY! Rainclaude fleet sunk. +50,000 Troops gained!</div>`;
      if (typeof GS !== 'undefined' && GS.players[GS.myIndex]) {
        GS.reinforceLeft += 5; 
        updateDashboard();
        if (typeof setLog === 'function') setLog("🎁 Naval Victory! +50,000 reserve troops added.");
      }
      if (typeof scene !== 'undefined' && window.navalWarState.boatMesh) {
         scene.remove(window.navalWarState.boatMesh);
         if (typeof boatMarkers !== 'undefined') {
            boatMarkers = boatMarkers.filter(b => b !== window.navalWarState.boatMesh);
         }
      }
    } else {
      log.innerHTML += `<div style="color:#f55; font-weight:bold; font-size:14px; margin-top:10px;">DEFEAT! Human fleet destroyed.</div>`;
      if (typeof setLog === 'function') setLog("💥 Naval Defeat! Fleet lost to Rainclaude.");
    }
    log.scrollTop = log.scrollHeight;
}
...
window.planeMarkers = [];
window.planeData = [];
window.shooterState = { active: false, ammo: 3, aiHp: 100, planeMesh: null };
window.satelliteWarState = { active: false, humanHp: 100, aiHp: 100, satMesh: null };

window.openSatelliteWar = function(mesh) {
  if (!window.isRedSkin) return;
  SFX.playClick();
  window.satelliteWarState = { active: true, humanHp: 100, aiHp: 100, satMesh: mesh };
  
  document.getElementById('sat-hp-human').textContent = '100';
  document.getElementById('sat-bar-human').style.width = '100%';
  document.getElementById('sat-hp-ai').textContent = '100';
  document.getElementById('sat-bar-ai').style.width = '100%';
  
  const log = document.getElementById('sat-log');
  log.innerHTML = '<div style="opacity:0.7;">Orbital target locked. Commencing cyber-attack...</div>';
  
  document.getElementById('btn-fire-laser').style.display = 'block';
  document.getElementById('satellite-war-modal').classList.add('show');
};

window.fireSatelliteLaser = function() {
  if (!window.satelliteWarState.active) return;
  SFX.playLaser(); // Future beam sound mapping, falls back to click if undefined
  
  // RNG Combat
  const humanDmg = Math.floor(15 + Math.random() * 20);
  const aiDmg = Math.floor(10 + Math.random() * 25);
  
  window.satelliteWarState.humanHp = Math.max(0, window.satelliteWarState.humanHp - aiDmg);
  window.satelliteWarState.aiHp = Math.max(0, window.satelliteWarState.aiHp - humanDmg);
  
  // Update UI
  document.getElementById('sat-hp-human').textContent = window.satelliteWarState.humanHp;
  document.getElementById('sat-bar-human').style.width = window.satelliteWarState.humanHp + '%';
  document.getElementById('sat-hp-ai').textContent = window.satelliteWarState.aiHp;
  document.getElementById('sat-bar-ai').style.width = window.satelliteWarState.aiHp + '%';
  
  const log = document.getElementById('sat-log');
  log.innerHTML += `<div><span style="color:#0af">HUMAN EMP hits for ${humanDmg} DMG!</span> | <span style="color:#f55">RAINCLAUDE EMP hits for ${aiDmg} DMG!</span></div>`;
  log.scrollTop = log.scrollHeight;
  
  // Check Win/Loss Condition
  if (window.satelliteWarState.humanHp <= 0 || window.satelliteWarState.aiHp <= 0) {
    window.satelliteWarState.active = false;
    document.getElementById('btn-fire-laser').style.display = 'none';
    
    if (window.satelliteWarState.humanHp > window.satelliteWarState.aiHp) {
      log.innerHTML += `<div style="color:#0f8; font-weight:bold; font-size:14px; margin-top:10px;">TACTICAL VICTORY! Rainclaude satellite neutralized. +20,000 Troops gained!</div>`;
      // Grant Reward
      if (GS.players[GS.myIndex]) {
        GS.reinforceLeft += 2; // 2 x 10k troops
        updateDashboard();
        setLog("🎁 Orbital Victory! +20,000 reserve troops added.");
      }
      
      // Remove satellite from map
      if (typeof scene !== 'undefined' && window.satelliteWarState.satMesh) {
         scene.remove(window.satelliteWarState.satMesh.userData.parent);
         if (typeof redSatMarkers !== 'undefined') {
            redSatMarkers = redSatMarkers.filter(b => b !== window.satelliteWarState.satMesh);
         }
      }
    } else if (window.satelliteWarState.aiHp > window.satelliteWarState.humanHp) {
      log.innerHTML += `<div style="color:#f55; font-weight:bold; font-size:14px; margin-top:10px;">DEFEAT! Human satellite destroyed.</div>`;
      setLog("💥 Orbital Defeat! Cyber-link severed.");
    } else {
      log.innerHTML += `<div style="color:#fa0; font-weight:bold; font-size:14px; margin-top:10px;">DRAW! Mutually assured network destruction.</div>`;
    }
    log.scrollTop = log.scrollHeight;
  }
};

window.spawnRedPlanes = function() {
  if (typeof scene === 'undefined' || typeof THREE === 'undefined') return;
  
  // Clear old planes
  if (window.planeData) {
     window.planeData.forEach(p => scene.remove(p.pivot));
  }
  window.planeMarkers = [];
  window.planeData = [];
  ...
  const R = typeof GLOBE_R !== 'undefined' ? GLOBE_R + 0.045 : 1.045; // Fly slightly higher than boats
  const planeMatNeutral = new THREE.MeshBasicMaterial({ color: 0xffffff, depthTest: true });
  const planeMatRaincloud = new THREE.MeshBasicMaterial({ color: 0xff0000, depthTest: true });
  const planeMatHumanity = new THREE.MeshBasicMaterial({ color: 0x0000ff, depthTest: true });
  
  // Build a generic airplane group (half the size of the boat markers)
  for (let i = 0; i < 60; i++) {
    let planeMat = planeMatNeutral;
    if (i < 18) { // 30% of 60
      planeMat = planeMatRaincloud;
    } else if (i < 54) { // 60% of 60
      planeMat = planeMatHumanity;
    }
    const planeGroup = new THREE.Group();
    
    // Plane flies in local -Y direction.
    // Up is +Z
    
    // Fuselage (Cylinder height is along Y)
    const bodyGeo = new THREE.CylinderGeometry(0.005, 0.005, 0.06, 8);
    const body = new THREE.Mesh(bodyGeo, planeMat);
    
    // Wings (Span X, thickness Z, chord length Y)
    const wingGeo = new THREE.BoxGeometry(0.08, 0.015, 0.002);
    const wings = new THREE.Mesh(wingGeo, planeMat);
    wings.position.set(0, -0.01, 0); // slightly forward
    
    // Tail
    const tailGeo = new THREE.BoxGeometry(0.025, 0.01, 0.002);
    const tail = new THREE.Mesh(tailGeo, planeMat);
    tail.position.set(0, 0.025, 0); // back towards +Y
    
    // Fin (Up +Z)
    const finGeo = new THREE.BoxGeometry(0.002, 0.01, 0.015);
    const fin = new THREE.Mesh(finGeo, planeMat);
    fin.position.set(0, 0.025, 0.0075);
    ...
    // Create pivot and position it
    const pivot = new THREE.Group();
    scene.add(pivot);
    
    // Add plane to pivot, offset by radius length (so local origin is at Globe's core)
    planeGroup.position.set(0, 0, R);
    
    // Ensure no extra rotations so travel axis matches perfectly
    planeGroup.rotation.set(0, 0, 0);
    
    pivot.add(planeGroup);
    
    // Randomize initial starting point on the globe
    pivot.rotation.set(
    ...
    // Generous but appropriately-sized invisible hitbox for clicking (0.12x0.12x0.12)
    const hitBoxGeo = new THREE.BoxGeometry(0.12, 0.12, 0.12);
    const hitBoxMat = new THREE.MeshBasicMaterial({visible: false});
    const hitBox = new THREE.Mesh(hitBoxGeo, hitBoxMat);
    planeGroup.add(hitBox);
    
    // Set user data for raycaster
    hitBox.userData = { isPlane: true, pivot: pivot };

    window.planeMarkers.push(hitBox); // Raycast against the hitbox
    
    // Save orbital mechanics (rotate around X-axis now that plane is offset on Z)
    // Rotating around +X causes movement in -Y locally (Nose First)
    const axis = new THREE.Vector3(1, 0, 0); 
    const speed = Math.random() * 0.002 + 0.001; 
    window.planeData.push({ pivot, axis, speed });
  }
};

window.openShooterGame = function(hitBoxMesh) {
  if (!window.isRedSkin) return;
  SFX.playClick();
  
  const parentPlane = hitBoxMesh.parent;
  window.shooterState = { active: true, ammo: 3, aiHp: 100, planeMesh: parentPlane };
  
  document.getElementById('shooter-ammo').textContent = '3';
  document.getElementById('shooter-hp-ai').textContent = '100';
  document.getElementById('shooter-bar-ai').style.width = '100%';
  
  const log = document.getElementById('shooter-log');
  log.innerHTML = '<div style="opacity:0.7;">RPG-7 armed. Target aircraft locked.</div>';
  
  document.getElementById('btn-fire-rpg').style.display = 'block';
  document.getElementById('shooter-modal').classList.add('show');
};

window.fireRPG = function() {
  if (!window.shooterState.active || window.shooterState.ammo <= 0) return;
  SFX.playClick();
  
  // Hide shooter modal and open the manual RPG mini-game overlay
  document.getElementById('shooter-modal').classList.remove('show');
  const screen = document.getElementById('rpg-aim-screen');
  screen.style.display = 'block';
  screen.style.background = 'radial-gradient(circle at center, rgba(100,150,250,0.6) 0%, rgba(10,20,40,0.95) 100%)';
  ...
  // Track mouse bounded to approx 1/3 viewport from center
  window._rpgMouseListener = function(e) {
      if (!window.rpgState.active) return;
      const mw = window.innerWidth / 2;
      const mh = window.innerHeight / 2;
      
      // Calculate offset from center
      let dX = e.clientX - mw;
      let dY = e.clientY - mh;
      
      // Clamp to 1/3 viewport
      const limitX = window.innerWidth / 3;
      const limitY = window.innerHeight / 3;
      
      if (dX > limitX) dX = limitX;
      if (dX < -limitX) dX = -limitX;
      if (dY > limitY) dY = limitY;
      if (dY < -limitY) dY = -limitY;
      
      window.rpgState.mouseX = dX;
      window.rpgState.mouseY = dY;
  };
  document.addEventListener('mousemove', window._rpgMouseListener);
  
  window.rpgLoop();
};

window.rpgLoop = function() {
  if (!window.rpgState.active) return;
  window.rpgState.time += 0.04; // Speed of the aircraft
  
  const t = window.rpgState.time;
  
  // Complex sweeping motion with erratic shaking
  window.rpgState.targetX = Math.sin(t * 1.2) * 400 + Math.sin(t * 3.5) * 80 + (Math.random() * 30 - 15);
  window.rpgState.targetY = Math.cos(t * 0.9) * 250 + Math.cos(t * 4.2) * 60 + (Math.random() * 30 - 15);
  
  const targetEl = document.getElementById('rpg-target');
  targetEl.style.transform = `translate(calc(-50% + ${window.rpgState.targetX}px), calc(-50% + ${window.rpgState.targetY}px)) rotate(${window.rpgState.targetX * 0.05 + 45}deg)`;
  
  // Move crosshair
  const crosshairEl = document.getElementById('rpg-crosshair');
  crosshairEl.style.transform = `translate(calc(-50% + ${window.rpgState.mouseX}px), calc(-50% + ${window.rpgState.mouseY}px))`;
  
  // Crosshair color feedback if hovering over target (relative distance)
  const relX = window.rpgState.targetX - window.rpgState.mouseX;
  const relY = window.rpgState.targetY - window.rpgState.mouseY;
  const dist = Math.sqrt(relX * relX + relY * relY);
  
  const dot = document.getElementById('rpg-crosshair-dot');
  if (dist < 60) {
     dot.style.background = '#0f8'; // Locked on
     dot.style.boxShadow = '0 0 15px #0f8';
  } else {
     dot.style.background = '#f00';
     dot.style.boxShadow = '0 0 10px #f00';
  }
  
  window.rpgState.animId = requestAnimationFrame(window.rpgLoop);
};

window.onRpgFire = function() {
  if (!window.rpgState.active) return;
  window.rpgState.active = false;
  cancelAnimationFrame(window.rpgState.animId);
  document.removeEventListener('mousemove', window._rpgMouseListener);
  ...
  const relY = window.rpgState.targetY - window.rpgState.mouseY;
  const dist = Math.sqrt(relX * relX + relY * relY);
  const isHit = dist < 65; // Tolerance radius (slightly larger than visual crosshair)
...
const SKINS = [
  { name: 'LIVE',  color: 0xffffff, btnText: '🔴 RED SKIN',    btnClass: '',              showOverlays: false, cleanGlobe: false, hideTroops: false, showResources: false, showArms: false, showBoats: false, isPremium: false },
  { name: 'RED',   color: 0xff2222, btnText: '🟢 GRN SKIN',  btnClass: 'active-normal', showOverlays: true,  cleanGlobe: false, hideTroops: false, showResources: false, showArms: false, showBoats: true, isPremium: false },
  { name: 'GREEN', color: 0x22ff22, btnText: '💠 CYBR SKIN',   btnClass: 'active-normal', showOverlays: false, cleanGlobe: true,  hideTroops: true,  showResources: true,  showArms: true, showBoats: false, isPremium: false },
  { name: 'CYBER', color: 0x00f0ff, btnText: '⬛ BLK SKIN',    btnClass: 'active-premium',showOverlays: false,  cleanGlobe: true, hideTroops: true,  showResources: false,  showArms: false,  showBoats: false,  isPremium: false },
  { name: 'BLK',   color: 0x050505, btnText: '🌍 LIVE SKIN',   btnClass: 'active-premium', showOverlays: false, cleanGlobe: false, hideTroops: true,  showResources: false, showArms: false, showBoats: false, isPremium: false }
];
window.SKINS = SKINS;
...
window.toggleRedSkin = function() {
  ...
  if (datacenterPowerBox) {
      datacenterPowerBox.style.display = (skin.name === 'CYBER') ? 'block' : 'none';
  }
...
window.mineD3XCoin = function() {
    ...
    SFX.playLaser(); // Drilling sound
    window.internalD3XMiningAmount += 0.05;
    
    const log = document.getElementById('mining-log');
    log.innerHTML += `<div><span style="color:#aaa">Energy Consumed.</span> Drill fired! Yielded <span style="color:#0f8">+0.05 D3X</span> progress.</div>`;
