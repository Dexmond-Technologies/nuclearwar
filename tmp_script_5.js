...
function handleGlobeClick(e) {
  ...
  // 1.1 Red Skin Webcam Dots Click Handler
  if (window.currentSkinIndex === 1 && window.webcamGroup && window.webcamGroup.visible && window.webcamGroup.children.length > 0) {
    mouse.x = (e.clientX/innerWidth)*2-1;
    mouse.y = -(e.clientY/innerHeight)*2+1;
    ray.setFromCamera(mouse, camera);
  ...
  if (currentSkinName === 'CYBER' && window.dexmondHubGroup && window.dexmondHubGroup.visible) {
      ...
      if (hubHits.length > 0) {
          if(typeof _sfxClick !== 'undefined') _sfxClick();
          openWeaponArsenal('DEXMOND', 'GLOBAL DEXMOND');
          return;
      }
  }
...
window.triggerSolanaDonation = async function() {
  ...
  if (window.currentSkinIndex === 3 && window.treasuryGroup && window.treasuryGroup.visible && window.treasuryGroup.children.length > 0) {
    ...
    if (treasuryHits.length > 0) {
      ...
      if (!tsModal) {
          // If the user hasn't defined the treasury modal UI yet, alert temporarily.
...
function updateGreenMarket() {
  ...
  Object.keys(window.greenMarket.commodities).forEach(k => {
    ...
    // Initialize basePrice if not set for dynamic floors
    ...
    if (typeof GS !== 'undefined' && GS.globalMetrics) {
        if (GS.globalMetrics.defcon <= 4 || (GS.catastrophes && GS.catastrophes.active)) {
            warVolMultiplier = 2.5; // War makes markets 2.5x more volatile
            
            // War Asset Correlation Bias
            if (k === 'Crude Oil' || k === 'Natural Gas' || k === 'Gold') {
                directionalDrift = 1.002; // Steady upward drift for Energy/Safe Havens
            } else if (k.includes('NVIDIA') || k.includes('TPU') || k.includes('EPYC')) {
                directionalDrift = 0.997; // Steady downward drift for Tech Hardware (Supply Chain Shock)
            }
        }
    }

    let change = 1 + (Math.random() * (c.vol * warVolMultiplier) * 2 - (c.vol * warVolMultiplier));
    // Apply directional correlation drift
    ...
    // Dynamic Price Floor (10% of original value, so Freshwater 0.10 can drop to 0.01 but not jump to 10)
    const floor = c.basePrice * 0.1;
    c.price = Math.max(floor, c.price * change);
...
window.tradeGreenMetal = function(metalName, amount) {
  ...
  const cost = c.price; // Native D3X cost
  const totalCost = Math.round(cost * Math.abs(amount) * 100) / 100;

  if (amount > 0) {
    if (window.localD3XBalance >= totalCost) {
      // Calculate Average Cost Basis
      let totalShares = c.shares + amount;
      let newTotalSpent = c.totalSpent + totalCost;
      ...
      window.greenMarket.tradeLogs.push(`[${tString}] <span style="color:#0f8">BOUGHT</span> 1 ${metalName} @ ${cost.toFixed(2)} D3X`);
    ...
    if (c.shares >= Math.abs(amount)) {
      ...
      let realizedPnL = (cost - c.avgCost) * Math.abs(amount);
      let pnlStr = realizedPnL >= 0 ? `<span style="color:#0f0">+$${realizedPnL.toFixed(2)} D3X</span>` : `<span style="color:#f00">-$${Math.abs(realizedPnL).toFixed(2)} D3X</span>`;
      
      c.totalSpent -= (c.avgCost * Math.abs(amount));
      ...
      const tString = new Date().toLocaleTimeString('en-US', {hour12:false, hour:'2-digit', minute:'2-digit', second:'2-digit'});
      window.greenMarket.tradeLogs.push(`[${tString}] <span style="color:#f55">SOLD</span> 1 ${metalName} @ ${cost.toFixed(2)} D3X | P&L: ${pnlStr}`);
      if (window.greenMarket.tradeLogs.length > 50) window.greenMarket.tradeLogs.shift();
  ...
  // 💰 PERSIST PORTFOLIO TO DB ON EVERY TRADE
  if (ws && ws.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({
          type: 'save_portfolio',
          callsign: window.myCallsign || GS.playerCallsign,
          portfolio: {
              commodities: window.greenMarket.commodities,
              tradeLogs: window.greenMarket.tradeLogs,
              localD3XBalance: window.localD3XBalance
          }
      }));
  }

  // Sync the top-right wallet balance UI
